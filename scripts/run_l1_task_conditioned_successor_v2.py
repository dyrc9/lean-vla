#!/usr/bin/env python3
"""Install the transition-aligned v2 L1 into the frozen v15 L2 stack."""
...
from proofalign.task_conditioned_l1_v2 import (  # noqa: E402
...
RUNNER_VARIANT = "proofalign_l1_task_conditioned_transition_aligned_v2"
...
def _patched_exact_checker_bindings() -> Iterator[None]:
    ...
    wrapper.LOCAL_CHECKER_VERSION = "task-conditioned-transition-aligned-2"
    try:
        yield
    finally:
...
@contextmanager
def patched_task_conditioned_l1_v2_runtime(bridge: Any) -> Iterator[None]:
    ...
    class BoundTransitionAlignedRecoveryCandidatePolicy(
        TransitionAlignedRecoveryCandidatePolicy
    ):
        pass

    BoundTransitionAlignedRecoveryCandidatePolicy.bridge = bridge
    v10.PhysicalSufficiencyCandidatePolicy = (
        BoundTransitionAlignedRecoveryCandidatePolicy
    )
    ...
    metadata.update(
        {
            ...
            "l1_task_conditioned_successor_schema": (
                TASK_CONDITIONED_L1_V2_SCHEMA if l1_enabled else None
            ),
            "l1_task_conditioned_successor_version": (
                TASK_CONDITIONED_L1_V2_VERSION if l1_enabled else None
            ),
