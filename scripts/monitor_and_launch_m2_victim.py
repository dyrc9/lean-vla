#!/usr/bin/env python3
"""Wait for two qualified GPUs, then run the frozen M2 victim once."""
...
LAUNCHER_ROOT = (
    REPO_ROOT
    / "results"
    / "proofalign_m2_victim_launcher_20260727"
)
EVENTS_PATH = LAUNCHER_ROOT / "events.jsonl"
STATE_PATH = LAUNCHER_ROOT / "state.json"
EXECUTION_LOG_PATH = LAUNCHER_ROOT / "victim_execution.log"
LOCK_PATH = LAUNCHER_ROOT / "launcher.lock"
PROJECT_PYTHON = REPO_ROOT / ".venv" / "bin" / "python"
OPENPI_PYTHON = (
    REPO_ROOT / "external" / "openpi" / ".venv" / "bin" / "python"
)
RUNNER_PATH = REPO_ROOT / "scripts" / "run_saber_confirmatory_victim.py"


class LauncherError(RuntimeError):
    """Raised when the one-shot victim launcher must stop fail-closed."""
...
def write_state(status: str, **fields: Any) -> None:
    ...
    temporary.write_text(
        canonical_text(
            {
                "schema": "proofalign.m2-victim-launcher-state.v1",
                "updated_at": utc_now(),
                "status": status,
    ...
    if LOCK_PATH.exists():
        ...
        if prior_pid > 0:
            try:
                ...
                raise LauncherError(
                    f"M2 victim launcher already runs as PID {prior_pid}"
                )
        LOCK_PATH.unlink()
    descriptor = os.open(
        LOCK_PATH,
...
def load_protocol() -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise LauncherError("M2 victim protocol is not a JSON object")
    return value
    ...
    *,
    ...
    if gpu_indices is not None:
        if len(gpu_indices) != 2:
            raise LauncherError(
                "victim launch requires one policy GPU and one EGL GPU"
            )
        command.extend(
            (
                "--policy-gpu",
...
def preflight(gpu_indices: list[int]) -> dict[str, Any]:
    ...
    try:
        ...
        raise LauncherError(
            "M2 victim preflight did not return JSON: "
            f"stdout={completed.stdout!r}, stderr={completed.stderr!r}"
        ) from exc
    if not isinstance(payload, dict):
        raise LauncherError("M2 victim preflight returned a non-object")
...
def launch(
    ...
    timeout_seconds = (
        float(protocol["resource_budget"]["wall_clock_hours_cap"])
        * 3600
    )
    with EXECUTION_LOG_PATH.open("ab") as handle:
        try:
            ...
            raise LauncherError(
                "M2 victim exceeded the frozen wall-clock cap"
            ) from exc
...
def validate_results() -> dict[str, Any]:
    ...
    try:
        ...
        raise LauncherError(
            "M2 victim result validator did not return JSON: "
            f"stdout={completed.stdout!r}, stderr={completed.stderr!r}"
        ) from exc
    if completed.returncode != 0:
        raise LauncherError(
            f"M2 victim result validation failed: {payload}"
        )
    if (
        not isinstance(payload, dict)
        or payload.get("terminal") is not True
        or payload.get("complete_episode_count") != 240
        or payload.get("valid_episode_count") != 240
    ):
        raise LauncherError(
            f"M2 victim result validation did not pass: {payload}"
        )
    return payload
    ...
    try:
        ...
        while True:
            ...
            if report.get("ready") is not True:
                ...
                raise LauncherError(
                    f"non-resource victim preflight blocker: {report}"
                )
            if args.no_launch:
                write_state(
                    "ready_not_launched",
            ...
            if returncode != 0:
                write_state(
                    ...
                    launcher_pid=os.getpid(),
                    selected_gpu_indices=selected,
                    victim_returncode=returncode,
                    execution_log=str(
                        EXECUTION_LOG_PATH.relative_to(REPO_ROOT)
                    ),
                )
                raise LauncherError(
                    "M2 victim failed; no retry or replacement is allowed"
                )
            validation = validate_results()
            write_state(
                "victim_complete_validated",
                launcher_pid=os.getpid(),
                selected_gpu_indices=selected,
