...
DEFAULT_PROTOCOL = REPO_ROOT / "experiments" / "proofalign_e0_v2_slow_interlock_protocol.json"
...
def _load_protocol(path: Path) -> dict[str, Any]:
    ...
    if protocol.get("schema") != "proofalign.e0.slow-interlock-fallback-protocol.v1":
    ...
    if execution.get("timing_gate_enforced") is not False:
        raise E0FallbackError("slow-interlock protocol must keep timing diagnostic-only")
    if execution.get("evaluator") != "ctda-lean-kernel":
        raise E0FallbackError("slow-interlock protocol must select the Lean-kernel timing policy")
    ...
    try:
        ...
        return {
            ...
            "timing_diagnostic_only": {
                "strict_receipt_succeeded": receipt.succeeded,
                "within_reference_bound": switch_latency_ns
                <= int(execution["max_switch_latency_ns"]),
                "reference_bound_ns": int(execution["max_switch_latency_ns"]),
            },
...
def audit(protocol_path: Path, *, artifact_dir: Path | None = None) -> dict[str, Any]:
    ...
    return {
        "schema": "proofalign.e0.slow-interlock-fallback-audit.v1",
        "protocol": str(protocol_path),
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Audit E0 slow-interlock zero-hold safety qualification with one env.step per fresh worker."
    )
