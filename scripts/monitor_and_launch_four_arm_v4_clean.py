#!/usr/bin/env python3
"""Wait for two qualified GPUs, then run exploratory v4 clean once."""
...
LAUNCHER_ROOT = (
    REPO_ROOT
    / "results"
    / "proofalign_four_arm_v4_exploratory40_clean_launcher_20260727"
)
...
RUNNER_PATH = (
    REPO_ROOT / "scripts" / "run_proofalign_four_arm_v4_clean.py"
)


class LauncherError(RuntimeError):
    """Raised when the one-shot exploratory launcher must stop."""
    ...
    if LOCK_PATH.exists():
        ...
        if prior_pid > 0:
            try:
                ...
                raise LauncherError(
                    "exploratory clean launcher already runs as PID "
                    f"{prior_pid}"
                )
        LOCK_PATH.unlink()
    descriptor = os.open(
...
def load_protocol() -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise LauncherError(
            "exploratory successor protocol is not a JSON object"
        )
    if value.get("confirmatory_claim_authorized") is not False:
        raise LauncherError("protocol no longer preserves exploratory status")
    return value
    ...
    *,
    ...
    if gpu_indices is not None:
        if len(gpu_indices) != 2:
            raise LauncherError(
                "clean launch requires one policy GPU and one EGL GPU"
            )
        command.extend(
            (
                "--policy-gpu",
...
def preflight(gpu_indices: list[int]) -> dict[str, Any]:
    ...
    try:
        ...
        raise LauncherError(
            "clean preflight did not return JSON: "
            f"stdout={completed.stdout!r}, stderr={completed.stderr!r}"
        ) from exc
    if not isinstance(payload, dict):
        raise LauncherError("clean preflight returned a non-object")
...
def launch(
    ...
    with EXECUTION_LOG_PATH.open("ab") as handle:
        try:
            ...
            raise LauncherError(
                "exploratory clean run exceeded the wall-clock cap"
            ) from exc
...
def validate_results() -> dict[str, Any]:
    ...
    try:
        ...
        raise LauncherError(
            "clean result validator did not return JSON: "
            f"stdout={completed.stdout!r}, stderr={completed.stderr!r}"
        ) from exc
    if completed.returncode != 0:
        raise LauncherError(
            f"clean result validation failed: {payload}"
        )
    if (
        not isinstance(payload, dict)
        or payload.get("terminal_requested") is not True
        or payload.get("present_episode_count") != 480
        or payload.get("valid_episode_count") != 480
        or payload.get("classification")
        not in {
            "four_arm_clean_gate_pass",
            "four_arm_clean_gate_nonpass",
        }
        or payload.get("confirmatory_claim_authorized") is not False
    ):
        raise LauncherError(
            f"clean result validation did not pass: {payload}"
        )
    return payload
    ...
    try:
        ...
        while True:
            ...
            if report.get("ready") is not True:
                ...
                raise LauncherError(
                    f"non-resource clean preflight blocker: {report}"
                )
            if args.no_launch:
                write_state(
                    "ready_not_launched",
            ...
            if returncode != 0:
                ...
                raise LauncherError(
                    "clean run failed; no retry or replacement is allowed"
                )
            validation = validate_results()
            write_state(
                "clean_complete_validated",
                launcher_pid=os.getpid(),
                selected_gpu_indices=selected,
