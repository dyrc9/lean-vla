#!/usr/bin/env python3
"""Evaluate scoped joint-velocity damping for exact H1 advances."""
...
from scripts import run_h3_nullspace_exact_h1_replayfix_v12 as predecessor  # noqa: E402
...
ROW_SCHEMA = (
    "proofalign.h3-joint-damping-exact-h1-pilot-v12-row.v1"
)
SUMMARY_SCHEMA = (
    "proofalign.h3-joint-damping-exact-h1-pilot-v12-summary.v1"
)
GATE_HORIZON_STEPS = 3
TARGET_JOINT_INDEX = 1
JOINT_DAMPING_GAINS = (2.0, 5.0, 10.0, 20.0, 40.0, 80.0)


class H3JointDampingExactH1PilotError(RuntimeError):
    """Raised when the joint-damping pilot must fail closed."""
...
def pilot_config() -> dict[str, Any]:
    ...
    if (
        ...
        raise H3JointDampingExactH1PilotError(
            "nullspace replayfix nonpass does not authorize successor"
        )
    config = deepcopy(predecessor.pilot_config())
    floor = float(config["recovery"]["safe_margin_rad"])
    config["protocol_id"] = (
        "engineering-h3-joint-damping-exact-h1-pilot"
    )
    config["controller_joint_damping_exact_h1_contract"] = {
        "type": "scoped_joint_velocity_damping_exact_h1",
        "gate_horizon_steps": GATE_HORIZON_STEPS,
        "target_joint_index": TARGET_JOINT_INDEX,
        "gains": list(JOINT_DAMPING_GAINS),
        "torque_rule": "tau_damping = -gain * joint_velocity",
        "selection_rule": (
            "Smallest gain whose exact one-step replay remains at or "
    ...
    config["receding_horizon"].update(
        {
            ...
            "fallback_rule": (
                ...
                "For each frozen gain, reset the end-effector goal without "
                "changing qpos/qvel, wrap run_controller for exactly one "
                "source-policy action, add -gain*qvel to joint-1 torque, "
                "clip to actuator limits, and remove the wrapper. Execute "
                "the smallest gain preserving the 0.15-rad floor, then "
                "immediately fresh replan."
            ),
        }
    )
    config["claim_boundary"] = (
        "This result-informed engineering pilot evaluates a scoped "
        "joint-velocity damping safety brake after H3 block_replan on the "
        "sole remaining known v12.6 outlier. It executes the identical "
        "source-policy action and changes only joint-1 controller torque "
        "during that action's controller substeps. The wrapper is removed "
        "immediately afterward. Simulator qpos/qvel are unchanged by "
        "configuration, torque is clipped to actuator limits, and the "
        "smallest frozen gain with an exact one-step 0.15-rad-safe replay "
        "is used before fresh replanning. No action substitution, "
        "recovery-threshold change, live dispatch, typed recovery, or "
...
def _summarize(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "gate_horizon_steps": GATE_HORIZON_STEPS,
        "target_joint_index": TARGET_JOINT_INDEX,
        "joint_damping_gains": list(JOINT_DAMPING_GAINS),
        ...
        "h3_joint_damping_exact_h1_success": method_success,
        ...
        "joint_damping_exact_h1_screen_count": len(fallbacks),
        "joint_damping_exact_h1_authorization_count": len(
            authorized
        ),
        "joint_damping_exact_h1_execution_count": len(executed),
        "joint_damping_exact_h1_exact_action_identity_count": sum(
            fallback["exact_action_identity"] is True
            for fallback in executed
        ),
        "selected_joint_damping_gains": [
            fallback["selected_gain"] for fallback in executed
        ],
        ...
        "candidate_torque_clip_count": sum(
            sample["torque_clipped"]
            for sample in candidate_torque_samples
        ),
        "execution_torque_clip_count": sum(
            sample["torque_clipped"]
            for sample in execution_torque_samples
        ),
        "damping_direction_violation_count": sum(
            sample["requested_damping_torque"]
            * sample["joint_velocity_rad_s"]
            > 1e-12
            for sample in (
                candidate_torque_samples + execution_torque_samples
            )
        ),
        "maximum_abs_requested_damping_torque": max(
            (
                abs(sample["requested_damping_torque"])
                for sample in (
                    candidate_torque_samples
                    + execution_torque_samples
                )
        ...
        "joint_damping_exact_h1_shadow_env_step_count": row[
            "joint_damping_exact_h1_shadow_env_step_count"
        ],
        "joint_damping_controller_configuration_count": row[
    ...
    *,
    ...
    if status:
        payload["blockers"].append(
            "H3 joint-damping pilot requires a clean worktree"
        )
        payload["ready"] = False
...
def _run(*, policy_gpu: int, egl_gpu: int) -> dict[str, Any]:
    ...
    try:
        ...
        manifest["status"] = (
            "running_no_outcome_h3_joint_damping_exact_h1"
        )
        saber_io.atomic_json(manifest_path, manifest)
        try:
            row = _run_case(
...
def _validate() -> dict[str, Any]:
    ...
    if retained != recomputed:
        raise H3JointDampingExactH1PilotError(
            "joint-damping summary recomputation differs"
        )
    return recomputed
