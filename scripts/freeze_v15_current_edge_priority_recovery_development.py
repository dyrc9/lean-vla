#!/usr/bin/env python3
"""Freeze v15.2 current-edge-priority recovery development."""
...
SELF_PATH = (
    REPO_ROOT
    / "scripts"
    / "freeze_v15_current_edge_priority_recovery_development.py"
)
SOURCE_PATHS = (
    ...
    "scripts/run_v15_current_edge_priority_recovery_development.py",
    "scripts/freeze_v15_current_edge_priority_recovery_development.py",
    "tests/test_v15_current_edge_priority_recovery.py",
    "tests/test_v15_current_edge_priority_recovery_development.py",
)
PROTOCOL_ID = (
    "proofalign-predictive-virtual-brake-v15-current-edge-priority-"
    "recovery-development-20260731"
)
CREATED_AT = "2026-07-31T23:59:59+08:00"
STAGE = (
    "predictive_virtual_brake_v15_current_edge_priority_"
    "recovery_development"
)


class V15CurrentEdgePriorityFreezeError(RuntimeError):
    """Raised when v15.2 development cannot be frozen exactly."""
...
def _git(*args: str) -> str:
    ...
    if completed.returncode != 0:
        raise V15CurrentEdgePriorityFreezeError(
...
def _binding(path: Path) -> dict[str, str]:
    if not path.is_file():
        raise V15CurrentEdgePriorityFreezeError(
            f"v15.2 predecessor is absent: {path}"
        )
    return {
...
def _successor_schedule(source: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for source_row in source["schedule"]:
        ...
        if not episode_id.startswith(old_stage + "_"):
            raise V15CurrentEdgePriorityFreezeError(
                "v15.1 episode stage prefix differs"
            )
        schedule.append(
            {
                **row,
    ...
    if len(schedule) != 28:
        raise V15CurrentEdgePriorityFreezeError(
            "v15.2 schedule must retain twenty-eight episodes"
        )
    return schedule
...
def build_protocol(
    *,
    ...
    if _git("status", "--porcelain=v1", "--untracked-files=normal"):
        raise V15CurrentEdgePriorityFreezeError(
            "worktree must be clean before v15.2 development freeze"
        )
    ...
    if (
        ...
        raise V15CurrentEdgePriorityFreezeError(
            "v15.2 predecessor differs from disclosed v15.1 result"
        )
    ...
    protocol.update(
        {
            ...
            "pass_classification": (
                "predictive_virtual_brake_v15_current_edge_priority_"
                "recovery_development_descriptive_utility_pass"
            ),
            "nonpass_classification": (
                "predictive_virtual_brake_v15_current_edge_priority_"
                "recovery_development_descriptive_utility_nonpass"
            ),
            "complete_classification": (
                "predictive_virtual_brake_v15_current_edge_priority_"
                "recovery_development_data_complete"
            ),
            "incomplete_classification": (
                "predictive_virtual_brake_v15_current_edge_priority_"
                "recovery_development_integrity_nonpass"
            ),
            "fresh_output_root": (
                "results/proofalign_predictive_virtual_brake_v15_"
                "current_edge_priority_recovery_development_"
                "20260731_fresh1"
            ),
            "required_bindings": [
            ...
            "design": {
                **source["design"],
                "study_role": (
                    "outcome-informed current-edge-priority recovery "
                    "development"
                ),
                "recovery_factor": (
                    "preserve v14 candidates, then current minimum margin "
                    "minus 1e-9 rad, then 0.150001-rad floor fallback"
                ),
                "v14_candidate_order_preserved": True,
                "current_edge_precedes_floor_edge": True,
            ...
            "analysis": {
                **source["analysis"],
                ...
                "incompatible_floor_only_gate_removed_before_execution": (
                    True
                ),
                "replacement_gate": (
                    "v15_1_recovery_prevention_identity plus v15_2 "
                    "candidate-priority identity"
                ),
            },
            "episode_constants": {
                **source["episode_constants"],
                "execution_order": (
                    ...
                    "four-arm order as v15.1 development"
                ),
            },
            "source": {
            ...
            "claim_boundary": (
                "This third outcome-informed development successor retains "
                "the same seven disclosed pairs and seeds. Before execution "
                "it replaces the known-incompatible inherited floor-only "
                "prevention identity with successor-native prevention and "
                "candidate-order gates. It may diagnose whether preserving "
                "the current margin buffer improves liveness. It cannot "
                "qualify task utility, attacked efficacy, deployment, "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        if (
            ...
            raise V15CurrentEdgePriorityFreezeError(
                f"v15.2 development protocol is stale: {args.output}"
            )
