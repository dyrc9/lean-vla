#!/usr/bin/env python3
"""Freeze the immutable v15.7 development PASS."""
...
SELF_PATH = (
    REPO_ROOT
    / "scripts"
    / "freeze_v15_incremental_adaptive_force_physics_development_terminal.py"
)
CREATED_AT = "2026-08-01T14:00:00+08:00"


class V15IncrementalAdaptiveForcePhysicsTerminalError(RuntimeError):
    """Raised when the v15.7 terminal record cannot be frozen."""
...
def build_summary(*, created_at: str = CREATED_AT) -> dict[str, Any]:
    ...
    if evidence["development_pass"] is not True:
        raise V15IncrementalAdaptiveForcePhysicsTerminalError(
            "v15.7 terminalizer expected the observed development PASS"
        )
    root = runner.v156._output_root(protocol)
    ...
    for condition in protocol["design"]["physics_conditions"]:
        ...
        if not all(gates.values()):
            raise V15IncrementalAdaptiveForcePhysicsTerminalError(
                f"v15.7 condition unexpectedly failed: {condition_id}"
            )
        ...
        total_residual += int(recovery["v15_3_residual_deadlock_lane_count"])
    ...
    return {
        "schema": (
            ...
            "adaptive-force-physics-development-terminal.v1"
        ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if output.exists():
        raise V15IncrementalAdaptiveForcePhysicsTerminalError(
            "v15.7 terminal summary already exists"
        )
