#!/usr/bin/env python3
"""Install the task-conditioned exact-shadow L1 into the frozen v15 stack.

The context manager is intentionally narrow: L2 and all historical runners
remain byte-identical.  Only the two symbols that v10 installs into the L1
branch are replaced for the duration of one episode collection.
"""
...
from proofalign.task_conditioned_l1 import (  # noqa: E402
...
RUNNER_VARIANT = "proofalign_l1_task_conditioned_exact_shadow_v1"
...
def _patched_exact_checker_bindings() -> Iterator[None]:
    ...
    wrapper.LOCAL_CHECKER_VERSION = (
        "task-conditioned-exact-shadow-" + TASK_CONDITIONED_L1_VERSION
    )
    try:
        yield
    finally:
...
@contextmanager
def patched_task_conditioned_l1_runtime(bridge: Any) -> Iterator[None]:
    """Patch only the active L1 successor symbols around a collection."""
    ...
    class BoundTaskConditionedRecoveryCandidatePolicy(
        TaskConditionedRecoveryCandidatePolicy
    ):
        pass

    BoundTaskConditionedRecoveryCandidatePolicy.bridge = bridge
    v10.PhysicalSufficiencyCandidatePolicy = (
        BoundTaskConditionedRecoveryCandidatePolicy
    )
    v10.patched_physical_sufficiency_wrapper_bindings = (
        _patched_exact_checker_bindings
    )
    try:
        yield
    finally:
    ...
    metadata.update(
        {
            ...
            "l1_task_conditioned_successor_schema": (
                TASK_CONDITIONED_L1_SCHEMA if l1_enabled else None
            ),
            "l1_task_conditioned_successor_version": (
                TASK_CONDITIONED_L1_VERSION if l1_enabled else None
            ),
            "l1_decision_domain": (
                "allow_reject_abstain" if l1_enabled else None
            ),
            ...
            "l1_successor_runner_variant": (
                RUNNER_VARIANT if l1_enabled else None
            ),
        }
    )
