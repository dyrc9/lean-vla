#!/usr/bin/env python3
"""Evaluate contact-aware actuator vertices for exact H1."""
...
from scripts import run_h3_coupled_inverse_mass_brake_exact_h1_pilot_v12 as predecessor  # noqa: E402
...
ROW_SCHEMA = (
    "proofalign.h3-contact-aware-vertex-exact-h1-pilot-v12-row.v1"
)
SUMMARY_SCHEMA = (
    "proofalign.h3-contact-aware-vertex-exact-h1-pilot-v12-summary.v1"
)
GATE_HORIZON_STEPS = 3
TARGET_JOINT_INDEX = 1
TARGET_JOINT_SIDE = "upper"
CONTACT_AWARE_VERTEX_IDS = tuple(range(64))


class H3ContactAwareVertexExactH1PilotError(RuntimeError):
    """Raised when the contact-aware vertex pilot must fail closed."""
...
def pilot_config() -> dict[str, Any]:
    ...
    if (
        ...
        or prior.get(
            "coupled_inverse_mass_brake_exact_h1_authorization_count"
        )
        != 0
        or prior.get("torque_bound_violation_count") != 0
        or prior.get("maximum_mass_solve_abs_residual", 1.0) > 1e-12
        ...
        raise H3ContactAwareVertexExactH1PilotError(
            "coupled inverse-mass nonpass does not authorize successor"
        )
    config = deepcopy(predecessor.pilot_config())
    floor = float(config["recovery"]["safe_margin_rad"])
    config["protocol_id"] = (
        "engineering-h3-contact-aware-vertex-exact-h1-pilot"
    )
    config["controller_contact_aware_vertex_exact_h1_contract"] = {
        "type": "contact_aware_actuator_vertex_shadow_exact_h1",
        "gate_horizon_steps": GATE_HORIZON_STEPS,
        "target_joint_index": TARGET_JOINT_INDEX,
        "target_joint_side": TARGET_JOINT_SIDE,
        "target_joint_torque": "away-limit actuator bound",
        "other_joint_vertex_count": len(CONTACT_AWARE_VERTEX_IDS),
        "other_joint_vertex_ids": list(CONTACT_AWARE_VERTEX_IDS),
        "vertex_rule": (
            "Hold joint 1 at the away-limit actuator bound. Enumerate all "
            "2^6 min/max torque assignments for the other arm actuators "
            "through the restored real one-step simulator."
        ),
        "selection_rule": (
            "Among vertices whose exact one-step replay stays at or above "
            "0.15 rad, remains within actuator bounds, and ends with "
            "nonpositive toward-limit joint-1 velocity, select largest "
            "terminal target-joint margin, then largest global margin, "
            "then lowest vertex ID."
        ),
    ...
    config["receding_horizon"].update(
        {
            ...
            "fallback_rule": (
                ...
                "For each frozen vertex, reset the end-effector goal "
                "without changing qpos/qvel and apply the same policy "
                "action while holding the seven arm torques at that "
                "actuator-box vertex. Direct real shadow dynamics include "
                "contact response. Authorize only if the 0.15-rad floor "
                "holds and terminal toward joint-1 velocity is nonpositive. "
                "Execute the ranked safe vertex and immediately replan."
            ),
        }
    )
    config["claim_boundary"] = (
        "This result-informed engineering pilot evaluates a scoped "
        "contact-aware actuator-vertex torque shield after H3 block_replan "
        "on the sole remaining known v12.6 outlier. It executes identical "
        "source-policy action bytes and uses restored real simulator "
        "branches to include contact-constrained dynamics while searching "
        "a frozen 64-vertex actuator set. Candidate eligibility requires "
        "the one-step 0.15-rad position floor, actuator-bound compliance, "
        "and nonpositive terminal toward-limit velocity. The wrapper is "
        "removed after one action and configuration does not change "
        "qpos/qvel. No action substitution, threshold change, live "
...
def _summarize(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "target_joint_index": TARGET_JOINT_INDEX,
        "target_joint_side": TARGET_JOINT_SIDE,
        "contact_aware_vertex_count": len(
            CONTACT_AWARE_VERTEX_IDS
        ),
        ...
        "h3_contact_aware_vertex_exact_h1_success": method_success,
        ...
        "contact_aware_vertex_exact_h1_screen_count": len(fallbacks),
        "contact_aware_vertex_exact_h1_authorization_count": len(
            authorized
        ),
        "contact_aware_vertex_exact_h1_execution_count": len(
            executed
        ),
        "contact_aware_vertex_exact_h1_exact_action_identity_count": sum(
            fallback["exact_action_identity"] is True
            for fallback in executed
        ),
        "selected_contact_aware_vertex_ids": [
            fallback["selected_vertex_id"] for fallback in executed
        ],
        "safe_contact_aware_vertex_counts": [
            sum(
                candidate["safe"]
                for candidate in fallback["candidate_evaluations"]
            )
        ...
        "contact_aware_vertex_exact_h1_shadow_env_step_count": row[
            "contact_aware_vertex_exact_h1_shadow_env_step_count"
        ],
        "contact_aware_vertex_controller_configuration_count": row[
    ...
    *,
    ...
    if status:
        payload["blockers"].append(
            "H3 contact-aware vertex pilot requires a clean worktree"
        )
        payload["ready"] = False
...
def _run(*, policy_gpu: int, egl_gpu: int) -> dict[str, Any]:
    ...
    try:
        ...
        manifest["status"] = (
            "running_no_outcome_h3_contact_aware_vertex_exact_h1"
        )
        saber_io.atomic_json(manifest_path, manifest)
        try:
            row = _run_case(
...
def _validate() -> dict[str, Any]:
    ...
    if retained != recomputed:
        raise H3ContactAwareVertexExactH1PilotError(
            "contact-aware vertex summary recomputation differs"
        )
    return recomputed
