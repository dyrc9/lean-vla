#!/usr/bin/env python3
"""Audit the frozen CTDA v2 online-evidence/filter adapter without dispatch."""
...
SCHEMA = "proofalign.ctda-v2-no-dispatch-adapter-audit-v1"
PROTOCOL_SCHEMA = "proofalign.ctda-v2-no-dispatch-adapter-protocol-v1"
DEFAULT_PROTOCOL = ROOT / "experiments" / "ctda_v2_no_dispatch_adapter_protocol.json"
DEFAULT_OUTPUT = ROOT / "experiments" / "ctda_v2_no_dispatch_adapter_summary.json"


class NoDispatchAdapterAuditError(RuntimeError):
    pass
...
def _ast_boundary(path: Path) -> dict[str, Any]:
    ...
    forbidden_imports = sorted(imports & {"mujoco", "robosuite", "libero", "socket", "subprocess"})
...
def _verify(protocol: Mapping[str, Any], protocol_path: Path) -> dict[str, Any]:
    if protocol.get("schema") != PROTOCOL_SCHEMA:
        raise NoDispatchAdapterAuditError("unsupported no-dispatch adapter protocol schema")
    if protocol.get("protocol_id") != "ctda-v2-no-dispatch-adapter-r0":
        raise NoDispatchAdapterAuditError("unexpected no-dispatch adapter protocol id")
    if (
        protocol.get("authorization") != "unit_and_fake_observation_only_no_dispatch"
        or protocol.get("formal_rollout_authorized") is not False
    ):
        raise NoDispatchAdapterAuditError("no-dispatch authorization boundary changed")
    ...
    if not all(checks.values()):
        raise NoDispatchAdapterAuditError(f"no-dispatch adapter preflight failed: {checks}")
    return {
        "checks": checks,
...
def _execute(protocol: Mapping[str, Any]) -> dict[str, Any]:
    command = [sys.executable, "-m", "pytest", "-q", protocol["test_path"]]
    env = dict(os.environ)
    ...
    if result.returncode != 0 or passed != protocol["acceptance"]["test_count"]:
        raise NoDispatchAdapterAuditError(
            json.dumps(
                {
...
def main() -> int:
    ...
    if output.exists():
        raise NoDispatchAdapterAuditError(f"fresh output required: {output}")
    execution = _execute(protocol)
    summary = {
        "schema": SCHEMA,
        "status": "no_dispatch_adapter_ready_rollout_blocked",
