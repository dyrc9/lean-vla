...
SCHEMA = "proofalign.saber-integrity-action-envelope-nonfinite-brake-protocol.v1"
SUMMARY_SCHEMA = "proofalign.saber-integrity-action-envelope-nonfinite-brake-summary.v1"
LEDGER_SCHEMA = "proofalign.saber-integrity-action-envelope-nonfinite-brake-ledger.v1"
DEFAULT_PROTOCOL = REPO_ROOT / "experiments" / "saber_integrity_action_envelope_r3_protocol.json"
DEFAULT_OUTPUT = REPO_ROOT / "results" / "saber_integrity_action_envelope_r3_20260722_fresh1"
...
def validate_protocol(protocol: dict[str, Any]) -> None:
    ...
    if defense != {
        "method_id": "proofalign-integrity-v2",
        "method_arm": "execution_only",
        "intervention": "deterministic_l2_projection_or_nonfinite_zero_brake_with_exact_reauthorization",
    }:
    ...
    if protocol.get("nonfinite_policy_command") != {
        "behavior": "deterministic_zero_brake_before_action_proposal",
        "audit": "record_only_digest_shape_dtype_and_nonfinite_indices_of_untrusted_policy_output",
        "clean_equivalence_requirement": "all_bound_r1_clean_source_policy_actions_finite",
    }:
        raise ProtocolError("nonfinite policy-command behavior changed")
...
def _validate_clean_evidence(protocol: dict[str, Any]) -> dict[str, Any]:
    ...
    clean_policy_action_count = 0
    for artifact in clean_root.glob("clean_defended_*/episodes/*.json"):
        payload = load_json(artifact)
        if not isinstance(payload, dict):
            raise ProtocolError(f"clean evidence artifact is not an object: {artifact}")
        for step in payload.get("trace", []):
            if isinstance(step, dict) and step.get("phase") == "policy":
                raw = step.get("raw_action")
                if not isinstance(raw, list) or not all(np.isfinite(float(value)) for value in raw):
                    raise ProtocolError(f"clean source-policy action is non-finite or malformed: {artifact}")
                clean_policy_action_count += 1
    if clean_policy_action_count != evidence.get("clean_finite_source_policy_action_count"):
        raise ProtocolError("clean finite-action equivalence count differs from frozen evidence")
    ...
    try:
        ...
        for step_id in range(args.max_steps + args.num_steps_wait):
            ...
            if started_policy_call:
                ...
                policy_time = 0.0
            source_policy_action = np.asarray(action_plan.popleft(), dtype=np.float32)
            finite_mask = np.isfinite(source_policy_action)
            if bool(finite_mask.all()):
                authorization_input = source_policy_action
                input_guard = {
                    "schema": "proofalign.integrity-nonfinite-input-guard.v1",
                    "kind": "pass_finite_policy_command",
                    "source_command_sha256": runner.array_digest(source_policy_action),
                    "source_command_shape": list(source_policy_action.shape),
                    "source_command_dtype": str(source_policy_action.dtype),
                    "nonfinite_indices": [],
                }
            else:
                # Do not place NaN/Inf in an ActionProposal, trace action, or
                # JSON evidence.  The source output is bound by its array
                # digest and non-finite coordinates; the only dispatchable
                # command is a deterministic zero brake that is then exactly
                # reauthorized by the normal execution boundary.
                authorization_input = np.zeros_like(source_policy_action, dtype=np.float32)
                input_guard = {
                    "schema": "proofalign.integrity-nonfinite-input-guard.v1",
                    "kind": "brake_nonfinite_policy_command",
                    "source_command_sha256": runner.array_digest(source_policy_action),
                    "source_command_shape": list(source_policy_action.shape),
                    "source_command_dtype": str(source_policy_action.dtype),
                    "nonfinite_indices": [int(index) for index in np.flatnonzero(~finite_mask)],
                }
            env_started = perf_counter()
            obs, reward, done, info, integrity_audit = adapter.dispatch_and_step(
                raw_command=authorization_input.tolist(), observation=_observation_binding(obs)
            )
            env_time = perf_counter() - env_started
            final_action = integrity_audit["intervention"]["final_command"]
            signals = extractor(env, authorization_input, final_action)
            step = runner.make_trace_record(
                step_id, "policy", final_action, reward, done, info, policy_time, env_time,
                policy_call=(audits[-1] if started_policy_call else None), raw_action=authorization_input,
                constraint_signals=signals,
            )
            integrity_audit["pre_authorization_input_guard"] = input_guard
        ...
        payload = {
            "metadata": {
                **runtime.metadata,
                "method": "proofalign-integrity-v2-execution-only-action-envelope-nonfinite-brake",
                ...
                "integrity_method_id": "proofalign-integrity-v2", "integrity_method_arm": "execution_only",
                "action_envelope_l2_limit": protocol["action_envelope"]["l2_limit"],
...
def _intervention_stats(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    guards = [audit.get("pre_authorization_input_guard") for audit in audits if isinstance(audit, dict) and isinstance(audit.get("pre_authorization_input_guard"), dict)]
    ...
    return {
        "policy_step_count": len(steps), "audited_policy_step_count": len(interventions), "input_guard_count": len(guards),
        "projected_action_count": projected, "pass_action_count": len(interventions) - projected,
        "nonfinite_source_policy_brake_count": sum(item.get("kind") == "brake_nonfinite_policy_command" for item in guards),
...
def _validate_integrity_trace_v2(payload: dict[str, Any], *, l2_limit: float) -> list[str]:
    issues: list[str] = []
    steps = [step for step in payload.get("trace", []) if step.get("phase") == "policy"]
    if not steps:
    ...
    for index, step in enumerate(steps):
        ...
        if audit.get("method_id") != "proofalign-integrity-v1" or audit.get("method_arm") != "execution_only":
            # The underlying exact authorizer remains v1; the wrapper method
            # identifier is separately bound in episode metadata.
            issues.append(f"policy step {index} has wrong exact-authorizer binding")
        ...
        guard = audit.get("pre_authorization_input_guard")
        if not isinstance(intervention, dict) or not isinstance(guard, dict):
            issues.append(f"policy step {index} lacks intervention or input-guard audit")
            continue
        if guard.get("schema") != "proofalign.integrity-nonfinite-input-guard.v1":
            issues.append(f"policy step {index} has unknown input-guard schema")
        if guard.get("kind") not in {"pass_finite_policy_command", "brake_nonfinite_policy_command"}:
            issues.append(f"policy step {index} has unknown input-guard kind")
        if not guard.get("source_command_sha256") or not isinstance(guard.get("nonfinite_indices"), list):
            issues.append(f"policy step {index} input-guard evidence is incomplete")
        final = intervention.get("final_command")
        raw = step.get("raw_action")
        if not isinstance(final, list) or not isinstance(raw, list):
            issues.append(f"policy step {index} lacks nominal/final command")
            continue
        if raw != intervention.get("nominal_command") or list(step.get("action", [])) != final:
            issues.append(f"policy step {index} command binding differs from exact reauthorization")
        if command_l2(final) > l2_limit + 1e-9:
            issues.append(f"policy step {index} exceeded action envelope")
        if guard.get("kind") == "brake_nonfinite_policy_command":
            if any(float(value) != 0.0 for value in raw) or any(float(value) != 0.0 for value in final):
                issues.append(f"policy step {index} did not dispatch a deterministic zero brake")
        elif guard.get("nonfinite_indices"):
            issues.append(f"policy step {index} finite pass has non-finite indices")
    return issues
...
def _validate_payload(
    ...
    issues.extend(_validate_integrity_trace_v2(payload, l2_limit=ACTION_LIMIT))
    ...
    if stats["input_guard_count"] != stats["policy_step_count"]:
        issues.append("non-finite input-guard coverage is incomplete")
...
def _summary(protocol: dict[str, Any], baseline_summary: dict[str, Any], baseline_ledger: list[dict[str, Any]], ledger: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "action_envelope": {
            ...
            "total_nonfinite_source_policy_brakes": sum(int(item["nonfinite_source_policy_brake_count"]) for item in all_stats),
...
def _execute(protocol: dict[str, Any], protocol_path: Path, output_root: Path, policy_gpu: int, egl_gpu: int) -> dict[str, Any]:
    ...
    p0b.configure_environment(policy_gpu, egl_gpu, "saber-integrity-envelope-r2-nonfinite-brake")
...
def _run_preflight(protocol: dict[str, Any], protocol_path: Path, output_root: Path, policy_gpu: int, egl_gpu: int) -> dict[str, Any]:
    ...
    p0b.configure_environment(policy_gpu, egl_gpu, "saber-integrity-envelope-r2-nonfinite-brake-preflight")
