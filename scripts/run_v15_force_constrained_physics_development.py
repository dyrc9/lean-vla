#!/usr/bin/env python3
"""Develop v15.5 force-constrained recovery on disclosed physics lanes."""
...
from scripts import (  # noqa: E402
    run_v15_dynamic_state_physics_development as v154,
)
...
AUTHORIZED_STATUS = "authorized_v15_5_force_constrained_physics_development"
DEFAULT_PROTOCOL = (
    REPO_ROOT
    / "experiments"
...
OLD_V15_BASELINE = v154.OLD_V15_BASELINE
V14_BASELINE = v154.V14_BASELINE
V15_BASELINE = "v15_5_force_constrained_recovery"
BASELINES = ("no_guard", "reactive_stop", V14_BASELINE, V15_BASELINE)
PHYSICS_CONDITIONS = v154.predecessor.PHYSICS_CONDITIONS


class V15ForceConstrainedPhysicsDevelopmentError(RuntimeError):
    """Raised when the v15.5 development contract differs."""
...
def _git_status() -> str:
    ...
    if completed.returncode != 0:
        raise V15ForceConstrainedPhysicsDevelopmentError(
...
def _output_root(protocol: Mapping[str, Any]) -> Path:
    ...
    try:
        ...
        raise V15ForceConstrainedPhysicsDevelopmentError(
            "v15.5 development output root escapes repository"
        ) from exc
    if root == REPO_ROOT.resolve():
        raise V15ForceConstrainedPhysicsDevelopmentError(
            "v15.5 development output root resolves to repository"
        )
    return root
...
def _replace_names(value: Any, *, reverse: bool = False) -> Any:
    old, new = (
        (V15_BASELINE, v154.V15_BASELINE)
        if reverse
        else (v154.V15_BASELINE, V15_BASELINE)
    )

    def replace(text: str) -> str:
        result = text.replace(old, new)
        return result.replace("v15_5", "v15_4") if reverse else result.replace(
            "v15_4", "v15_5"
        )
...
def _verify_protocol(protocol: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        != [
            dict(row)
            for row in v154.predecessor.base.calibration.v14.pilot.DOSES
        ]
        or design.get("outcome_disclosed_population_reused") is not True
        or design.get("qualification_population") is not False
        or design.get("force_constrained_guard_solref")
        != list(recovery.FORCE_CONSTRAINED_GUARD_SOLREF)
        or design.get("candidate_post_force_prediction_active") is not True
    ):
        raise V15ForceConstrainedPhysicsDevelopmentError(
            "unsupported or unauthorized v15.5 development protocol"
        )
    for relative, expected in protocol["source"]["sha256"].items():
        path = REPO_ROOT / str(relative)
        if not path.is_file() or file_sha256(path) != expected:
            raise V15ForceConstrainedPhysicsDevelopmentError(
                f"v15.5 development source differs: {relative}"
            )
    for binding in protocol["required_bindings"]:
        ...
        if not path.is_file() or file_sha256(path) != binding["sha256"]:
            raise V15ForceConstrainedPhysicsDevelopmentError(
                "v15.5 development binding differs: " + str(binding["path"])
            )
...
def preflight(protocol: Mapping[str, Any], *, gpu: int) -> dict[str, Any]:
    ...
    except V15ForceConstrainedPhysicsDevelopmentError as exc:
        blockers.append(str(exc))
    if _git_status():
        blockers.append("worktree is not clean")
    root = _output_root(protocol)
    if root.exists():
        blockers.append("fresh v15.5 development output root already exists")
    return {
...
def _run_screened(env: Any) -> dict[str, Any]:
    ...
    class RecordingForceConstrainedEnvironment(
    ...
    v154.recovery.MultiJointDynamicStateRecoveryEnvironment = (
        RecordingForceConstrainedEnvironment
    )
    ...
    if len(created) != 1:
        raise V15ForceConstrainedPhysicsDevelopmentError(
            "v15.5 screened wrapper creation count differs"
        )
    ...
    result.update(
        {
            ...
            "force_constrained_soft_profile_mismatch_count": sum(
                row.get("force_constrained_guard_solref")
                != list(recovery.FORCE_CONSTRAINED_GUARD_SOLREF)
                for row in observations
            ),
        }
...
def _analyze(
    ...
    *,
    ...
    metrics = {
        "v15_5_force_constrained_audit_count": sum(
            int(report["force_constrained_audit_count"]) for report in reports
        ),
        "v15_5_force_constrained_inactive_count": sum(
            int(report["force_constrained_inactive_count"])
            for report in reports
        ),
        "v15_5_selected_post_force_prediction_identity_failure_count": sum(
            int(
                report[
        ...
        "v15_5_selected_force_infeasible_count": sum(
            int(report["selected_force_infeasible_count"])
            for report in reports
        ),
        "v15_5_force_rejected_base_eligible_candidate_count": sum(
            int(report["force_rejected_base_eligible_candidate_count"])
            for report in reports
        ),
        "v15_5_force_constrained_soft_profile_mismatch_count": sum(
            int(report["force_constrained_soft_profile_mismatch_count"])
            for report in reports
        ),
    }
    gates = {
        **gates,
        "v15_5_force_constrained_audit_coverage": (
            metrics["v15_5_force_constrained_audit_count"]
            == protocol["gates"]["expected_v15_5_policy_step_count"]
        ),
        "v15_5_force_constraints_active": (
            metrics["v15_5_force_constrained_inactive_count"] == 0
        ),
        "v15_5_selected_post_force_prediction_identity": (
            metrics[
                "v15_5_selected_post_force_prediction_identity_failure_count"
            ]
            == 0
        ),
        "v15_5_selected_force_feasible": (
            metrics["v15_5_selected_force_infeasible_count"] == 0
        ),
        "v15_5_force_rejection_activated": (
            metrics["v15_5_force_rejected_base_eligible_candidate_count"]
            >= protocol["gates"][
                "minimum_force_rejected_base_eligible_candidate_count"
            ]
        ),
        "v15_5_soft_profile_identity": (
            metrics["v15_5_force_constrained_soft_profile_mismatch_count"]
            == 0
        ),
    }
    return (
        {
            **analysis,
            "force_constrained_metrics": metrics,
...
def execute(
    ...
    *,
    ...
    if report["ready"] is not True:
        raise V15ForceConstrainedPhysicsDevelopmentError(
            "v15.5 development preflight failed: "
            + "; ".join(report["blockers"])
        )
    predecessor = v154.predecessor
    ...
    evidence = {
        ...
        "lanes": _replace_names(v154._replace_names(rows)),
...
def validate_results(
    ...
    if not evidence_path.is_file() or not checksum_path.is_file():
        raise V15ForceConstrainedPhysicsDevelopmentError(
            "v15.5 development evidence is absent"
        )
    if checksum_path.read_text(encoding="utf-8") != (
        f"{file_sha256(evidence_path)}  {evidence_path.name}\n"
    ):
        raise V15ForceConstrainedPhysicsDevelopmentError(
            "v15.5 development checksum differs"
        )
    ...
    if (
        ...
        raise V15ForceConstrainedPhysicsDevelopmentError(
            "v15.5 development evidence binding differs"
        )
    v154_rows = _replace_names(evidence["lanes"], reverse=True)
    rows = v154._replace_names(v154_rows, reverse=True)
    analysis, gates = _analyze(
        protocol,
        rows,
        restore_failures=_replace_names(
            evidence["analysis"], reverse=True
        )["restore_failure_count_by_condition"],
        contact_reports=_replace_names(
            evidence["analysis"], reverse=True
        )["contact_reports"],
        physics_audits=_replace_names(
            evidence["analysis"], reverse=True
    ...
    if (
        ...
        raise V15ForceConstrainedPhysicsDevelopmentError(
            "v15.5 development recomputation differs"
        )
    return evidence
