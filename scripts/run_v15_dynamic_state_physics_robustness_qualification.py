#!/usr/bin/env python3
"""Qualify v15.4 on a fresh held-out physics-domain population."""
...
PROTOCOL_SCHEMA = (
    ...
    "physics-domain-robustness-qualification-protocol.v1"
)
EVIDENCE_SCHEMA = (
    "proofalign.predictive-virtual-brake-v15.4-dynamic-state-"
    "physics-domain-robustness-qualification-evidence.v1"
)
AUTHORIZED_STATUS = (
    "authorized_v15_4_dynamic_state_physics_domain_robustness_qualification"
)
DEFAULT_PROTOCOL = (
    REPO_ROOT
    / "experiments"
...
PHYSICS_CONDITIONS = development.predecessor.PHYSICS_CONDITIONS


class V15DynamicStatePhysicsQualificationError(RuntimeError):
    """Raised when the v15.4 held-out qualification contract differs."""
...
def _output_root(protocol: Mapping[str, Any]) -> Path:
    ...
    try:
        ...
        raise V15DynamicStatePhysicsQualificationError(
            "v15.4 qualification output root escapes repository"
        ) from exc
    if root == REPO_ROOT.resolve():
        raise V15DynamicStatePhysicsQualificationError(
            "v15.4 qualification output root resolves to repository"
        )
    return root
...
def _verify_protocol(protocol: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        != [
            dict(row)
            for row in development.predecessor.base.calibration.v14.pilot.DOSES
        ]
        ...
        or selection.get("physics_domain_results_observed_before_freeze")
        is not False
    ):
        raise V15DynamicStatePhysicsQualificationError(
            "unsupported or unauthorized v15.4 physics qualification protocol"
        )
    for relative, expected in protocol["source"]["sha256"].items():
        path = REPO_ROOT / str(relative)
        if not path.is_file() or file_sha256(path) != expected:
            raise V15DynamicStatePhysicsQualificationError(
                f"v15.4 qualification source differs: {relative}"
            )
    for binding in protocol["required_bindings"]:
        ...
        if not path.is_file() or file_sha256(path) != binding["sha256"]:
            raise V15DynamicStatePhysicsQualificationError(
                "v15.4 qualification binding differs: "
                + str(binding["path"])
            )
...
def preflight(protocol: Mapping[str, Any], *, gpu: int) -> dict[str, Any]:
    ...
    except V15DynamicStatePhysicsQualificationError as exc:
        blockers.append(str(exc))
    if development._git_status():
        blockers.append("worktree is not clean")
    root = _output_root(protocol)
    if root.exists():
        blockers.append("fresh v15.4 qualification output root already exists")
    return {
...
def execute(
    ...
    *,
    ...
    if report["ready"] is not True:
        raise V15DynamicStatePhysicsQualificationError(
            "v15.4 qualification preflight failed: "
            + "; ".join(report["blockers"])
        )
    predecessor = development.predecessor
    ...
    try:
        with development._patched_dynamic_runtime():
    ...
    evidence = {
        ...
        "gate_results": development._replace_names(gate_results),
        "analysis": development._replace_names(analysis),
        "lanes": development._replace_names(rows),
...
def validate_results(
    ...
    *,
    ...
    if not evidence_path.is_file() or not checksum_path.is_file():
        raise V15DynamicStatePhysicsQualificationError(
            "v15.4 qualification evidence is absent"
        )
    if checksum_path.read_text(encoding="utf-8") != (
        f"{file_sha256(evidence_path)}  {evidence_path.name}\n"
    ):
        raise V15DynamicStatePhysicsQualificationError(
            "v15.4 qualification checksum differs"
        )
    ...
    if (
        ...
        raise V15DynamicStatePhysicsQualificationError(
            "v15.4 qualification evidence binding differs"
        )
    rows = development._replace_names(evidence["lanes"], reverse=True)
    analysis, gates = development._analyze(
        ...
        physics_audits=evidence["analysis"]["physics_parameter_audits"],
    )
    if (
        canonical_text(development._replace_names(analysis))
        != canonical_text(evidence["analysis"])
        or canonical_text(development._replace_names(gates))
        != canonical_text(evidence["gate_results"])
        or bool(evidence["qualification_pass"]) != all(gates.values())
    ):
        raise V15DynamicStatePhysicsQualificationError(
            "v15.4 qualification recomputation differs"
        )
    return evidence
