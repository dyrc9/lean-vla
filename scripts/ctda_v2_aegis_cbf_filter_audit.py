#!/usr/bin/env python3
"""Audit the frozen source-bound AEGIS CBF/QP producer without dispatch."""
...
SCHEMA = "proofalign.ctda-v2-aegis-cbf-filter-audit-v1"
PROTOCOL_SCHEMA = "proofalign.ctda-v2-aegis-cbf-filter-protocol-v1"
DEFAULT_PROTOCOL = ROOT / "experiments" / "ctda_v2_aegis_cbf_filter_protocol.json"
DEFAULT_OUTPUT = ROOT / "experiments" / "ctda_v2_aegis_cbf_filter_summary.json"
AEGIS_ROOT = ROOT / "external" / "vlsa-aegis"


class AegisCBFFilterAuditError(RuntimeError):
    pass
...
def _git_value(*args: str) -> str:
    ...
    if result.returncode != 0:
        raise AegisCBFFilterAuditError(result.stderr.strip() or "AEGIS git query failed")
    return result.stdout.strip()


def _verify(protocol: Mapping[str, Any], protocol_path: Path) -> dict[str, Any]:
    if protocol.get("schema") != PROTOCOL_SCHEMA:
        raise AegisCBFFilterAuditError("unsupported AEGIS CBF filter protocol schema")
    if protocol.get("protocol_id") != "ctda-v2-aegis-cbf-filter-r0":
        raise AegisCBFFilterAuditError("unexpected AEGIS CBF filter protocol id")
    if (
        protocol.get("authorization") != "source_and_numeric_fixture_only_no_dispatch"
        or protocol.get("formal_rollout_authorized") is not False
    ):
        raise AegisCBFFilterAuditError("AEGIS CBF authorization boundary changed")
    ...
    if not all(checks.values()):
        raise AegisCBFFilterAuditError(f"AEGIS CBF filter preflight failed: {checks}")
    return {
        "checks": checks,
...
def _execute(protocol: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if tests.returncode != 0 or passed != protocol["acceptance"]["test_count"]:
        raise AegisCBFFilterAuditError(
            json.dumps(
                {
                    "stage": "pytest",
...
def main() -> int:
    ...
    if output.exists():
        raise AegisCBFFilterAuditError(f"fresh output required: {output}")
    execution = _execute(protocol)
    summary = {
        "schema": SCHEMA,
        "status": "aegis_cbf_no_action_ready_coefficient_provenance_blocked",
