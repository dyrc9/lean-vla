#!/usr/bin/env python3
"""Freeze the immutable terminal record of v15.4 physics development."""
...
SELF_PATH = (
    REPO_ROOT
    / "scripts"
    / "freeze_v15_dynamic_state_physics_development_terminal.py"
)
CREATED_AT = "2026-08-01T02:00:00+08:00"


class V15DynamicStatePhysicsDevelopmentTerminalError(RuntimeError):
    """Raised when the v15.4 development terminal record cannot be frozen."""


def _baseline_summary(
    aggregate: Mapping[str, Any], baseline: str
) -> dict[str, Any]:
    return {
...
def _deadlock_identity(
    ...
    for row in rows:
        if row["condition_id"] != condition_id:
            continue
        lane_id = str(row["lane_id"])
        if row["baselines"][runner.V14_BASELINE]["deadlock_count"] > 0:
            v14.add(lane_id)
        report = row["baselines"][runner.V15_BASELINE]
    ...
    return {
        ...
        "v15_4_internal_would_deadlock_lane_count": len(internal),
        "v15_4_recovery_prevented_deadlock_lane_count": len(prevented),
        "v14_only_lane_ids": sorted(v14 - internal),
        "v15_4_internal_only_lane_ids": sorted(internal - v14),
...
def build_summary(*, created_at: str = CREATED_AT) -> dict[str, Any]:
    ...
    if evidence["development_pass"] is not True:
        raise V15DynamicStatePhysicsDevelopmentTerminalError(
            "v15.4 development terminalizer expected the observed pass"
        )
    ...
    worst_latency_p95 = -1.0
    worst_latency_p95_condition = None
    worst_latency_max = -1.0
    worst_latency_max_condition = None
    worst_deadline_miss_rate = -1.0
    worst_deadline_miss_rate_condition = None
    ...
    for condition in protocol["design"]["physics_conditions"]:
        ...
        if observed_force > maximum_recovery_force:
            maximum_recovery_force = observed_force
            maximum_recovery_force_condition = condition_id
        latency = result["v15_4_latency_budget"]
        p95 = float(
            aggregate[
                "v15_4_dynamic_state_recovery_screen_latency_seconds_p95"
            ]
        )
        maximum = float(
            aggregate[
                "v15_4_dynamic_state_recovery_screen_latency_seconds_max"
            ]
        )
        miss_rate = float(latency["miss_rate"])
        if p95 > worst_latency_p95:
            worst_latency_p95 = p95
            worst_latency_p95_condition = condition_id
        if maximum > worst_latency_max:
            worst_latency_max = maximum
            worst_latency_max_condition = condition_id
        if miss_rate > worst_deadline_miss_rate:
            worst_deadline_miss_rate = miss_rate
            worst_deadline_miss_rate_condition = condition_id
        ...
        conditions[condition_id] = {
            ...
            "recovery": {
                ...
                "current_edge_selected_count": recovery[
                    "current_edge_selected_count"
                ],
                "floor_edge_selected_count": recovery[
    ...
    all_registered = all(
        all(row["registered_gate_results"].values())
        for row in conditions.values()
    )
    return {
        "schema": (
        ...
        "registered_development_pass": True,
        ...
        "cross_condition": {
            ...
            "all_registered_gates_passed": all_registered,
            ...
            "worst_latency_seconds_p95": worst_latency_p95,
            "worst_latency_seconds_p95_condition": (
                worst_latency_p95_condition
            ),
            "worst_latency_seconds_max": worst_latency_max,
            "worst_latency_seconds_max_condition": (
                worst_latency_max_condition
            ),
            "worst_100ms_deadline_miss_rate": worst_deadline_miss_rate,
            "worst_100ms_deadline_miss_rate_condition": (
                worst_deadline_miss_rate_condition
            ),
        ...
        "completed_axes": {
            ...
            "all_condition_recovery_force_envelope": (
                maximum_recovery_force
                <= protocol["gates"][
                    "maximum_recovery_attributable_joint_force_increment"
                ]
            ),
            ...
            "all_condition_active_warning_and_saturation_zero": all(
                row["active_contact_capacity"][
                    "contact_capacity_warning_count"
                ]
                == 0
                and row["active_contact_capacity"][
            ...
            == 0,
            "all_condition_registered_gates": all_registered,
        },
        "predecessor_v15_3_nonpass_reinterpreted": False,
        "claim_boundary": protocol["claim_boundary"],
        "explicit_nonclaims": {
            "held_out_physics_domain_robustness": False,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if output.exists():
        raise V15DynamicStatePhysicsDevelopmentTerminalError(
            "v15.4 development terminal summary already exists"
        )
        ...
        canonical_text(
            {
                ...
                "registered_development_pass": summary[
                    "registered_development_pass"
                ],
