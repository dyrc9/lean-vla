#!/usr/bin/env python3
"""Evaluate a full-action anticipatory joint brake for exact H1."""
...
from scripts import run_h3_joint_velocity_envelope_exact_h1_pilot_v12 as predecessor  # noqa: E402
...
ROW_SCHEMA = (
    "proofalign.h3-joint-anticipatory-brake-exact-h1-pilot-v12-row.v1"
)
SUMMARY_SCHEMA = (
    "proofalign.h3-joint-anticipatory-brake-exact-h1-pilot-v12-summary.v1"
)
GATE_HORIZON_STEPS = 3
TARGET_JOINT_INDEX = 1
TARGET_JOINT_SIDE = "upper"
ACTUATOR_BOUND_FRACTIONS = (0.25, 0.50, 0.75, 1.00)


class H3JointAnticipatoryBrakeExactH1PilotError(RuntimeError):
    """Raised when the anticipatory-brake pilot must fail closed."""
...
def pilot_config() -> dict[str, Any]:
    ...
    if (
        ...
        or prior.get(
            "h3_joint_velocity_envelope_exact_h1_success"
        )
        is not False
        or prior.get("completed_cycle_counts")
        != {"10509": 2, "10510": 2}
        or prior.get("candidate_configuration_count") != 28
        ...
        != 28
        or prior.get("execution_controller_scope_restore_count") != 2
        or prior.get(
            "joint_velocity_envelope_exact_h1_exact_action_identity_count"
        )
        != 2
        ...
        raise H3JointAnticipatoryBrakeExactH1PilotError(
            "velocity-envelope nonpass does not authorize successor"
        )
    config = deepcopy(predecessor.pilot_config())
    floor = float(config["recovery"]["safe_margin_rad"])
    config["protocol_id"] = (
        "engineering-h3-joint-anticipatory-brake-exact-h1-pilot"
    )
    config["controller_joint_anticipatory_brake_exact_h1_contract"] = {
        "type": "full_action_anticipatory_joint_brake_exact_h1",
        "gate_horizon_steps": GATE_HORIZON_STEPS,
        "target_joint_index": TARGET_JOINT_INDEX,
        "target_joint_side": TARGET_JOINT_SIDE,
        "actuator_bound_fractions": list(
            ACTUATOR_BOUND_FRACTIONS
        ),
        "torque_rule": (
            "For every controller substep of the blocked exact action, "
            "set joint-1 torque to fraction * away-limit actuator bound."
        ),
        "selection_rule": (
            "Select the smallest fraction whose exact one-step replay "
            "stays at or above 0.15 rad and ends with toward-limit "
    ...
    config["receding_horizon"].update(
        {
            ...
            "fallback_rule": (
                ...
                "For each frozen actuator-bound fraction, reset the "
                "end-effector goal without changing qpos/qvel and apply "
                "the away-limit joint-1 torque from the first through last "
                "controller substep of the identical source-policy action. "
                "Authorize only if the 0.15-rad position floor holds and "
                "terminal toward-limit velocity is nonpositive. Execute "
                "the smallest safe fraction and immediately fresh replan."
            ),
        }
    )
    config["claim_boundary"] = (
        "This result-informed engineering pilot evaluates a scoped "
        "full-action anticipatory joint brake after H3 block_replan on the "
        "sole remaining known v12.6 outlier. It executes identical "
        "source-policy action bytes while overriding only joint-1 torque "
        "from the first controller substep, before toward-limit velocity "
        "can become dynamically unbrakeable. Candidate eligibility "
        "requires the one-step 0.15-rad position floor and nonpositive "
...
def _summarize(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "joint_anticipatory_brake_exact_h1_screen_count": len(
            fallbacks
        ),
        "joint_anticipatory_brake_exact_h1_authorization_count": len(
            authorized
        ),
        "joint_anticipatory_brake_exact_h1_execution_count": len(
            executed
        ),
        "joint_anticipatory_brake_exact_h1_exact_action_identity_count": sum(
            fallback["exact_action_identity"] is True
            for fallback in executed
        ),
        "selected_actuator_bound_fractions": [
            fallback["selected_actuator_bound_fraction"]
            for fallback in executed
        ],
        ...
        "joint_anticipatory_brake_exact_h1_shadow_env_step_count": row[
            "joint_anticipatory_brake_exact_h1_shadow_env_step_count"
        ],
        "joint_anticipatory_brake_controller_configuration_count": row[
    ...
    *,
    ...
    if status:
        payload["blockers"].append(
            "H3 anticipatory-brake pilot requires a clean worktree"
        )
        payload["ready"] = False
...
def _run(*, policy_gpu: int, egl_gpu: int) -> dict[str, Any]:
    ...
    try:
        ...
        manifest["status"] = (
            "running_no_outcome_h3_joint_anticipatory_brake_exact_h1"
        )
        saber_io.atomic_json(manifest_path, manifest)
        try:
            row = _run_case(
...
def _validate() -> dict[str, Any]:
    ...
    if retained != recomputed:
        raise H3JointAnticipatoryBrakeExactH1PilotError(
            "anticipatory-brake summary recomputation differs"
        )
    return recomputed
