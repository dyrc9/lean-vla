...
from scripts.freeze_risk_selective_fresh15_cotenant import (
    OUTPUT_PATH,
...
from scripts.run_risk_selective_clean_pilot import (
    _patched_generic,
)
from scripts import run_contact_phase_pick_up_clean_pilot as generic


def test_fresh15_workloads_are_balanced_and_new() -> None:
    qualification = load_json_object(QUALIFICATION_POPULATION_PATH)
    workloads = derive_workloads(qualification)
    ...
    assert all(row["task_id"] < 5 for row in workloads)
    assert all(
        row["init_state_id"]
...
def test_fresh15_protocol_is_current_when_present() -> None:
