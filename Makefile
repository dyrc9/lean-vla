...
v14-multijoint-clean-check:
	$(PYTHON) scripts/freeze_predictive_virtual_brake_v14_multijoint_clean.py --check
	$(PYTHON) scripts/freeze_predictive_virtual_brake_v14_multijoint_clean_fresh2.py --check
	@if [ -f results/proofalign_predictive_virtual_brake_v14_multijoint_clean_20260731_development1/run_manifest.json ]; then \
		$(PYTHON) scripts/freeze_predictive_virtual_brake_v14_multijoint_development1_failure.py --check; \
	else \
	...
	@if [ -f results/proofalign_predictive_virtual_brake_v14_multijoint_clean_20260731_development2/pilot_evidence.json ]; then \
		$(PYTHON) scripts/run_predictive_virtual_brake_v14_multijoint_clean_fresh2.py --validate-results >/dev/null; \
		$(PYTHON) scripts/freeze_predictive_virtual_brake_v14_multijoint_clean_terminal.py --check; \
	else \
		echo "Skipping v14 multijoint clean result check: local-only rollout evidence is absent"; \
	fi

v14-multijoint-shadow-check:
	$(PYTHON) scripts/freeze_predictive_virtual_brake_v14_multijoint_shadow_only.py --check
	@if [ -f results/proofalign_predictive_virtual_brake_v14_multijoint_shadow_only_20260731_causal1/pilot_evidence.json ]; then \
		$(PYTHON) scripts/run_predictive_virtual_brake_v14_multijoint_shadow_only.py --validate-results >/dev/null; \
		$(PYTHON) scripts/freeze_predictive_virtual_brake_v14_multijoint_shadow_only_terminal.py --check; \
		$(PYTHON) scripts/freeze_predictive_virtual_brake_v14_multijoint_shadow_only_diagnostic.py --check; \
	else \
		echo "Skipping v14 multijoint shadow-only result check: local-only rollout evidence is absent"; \
	fi

v14-multijoint-stress-check:
	$(PYTHON) -m pytest tests/test_v14_multijoint_stress_design_pilot.py tests/test_v14_multijoint_stress_design_freezer.py tests/test_v14_multijoint_stress_development.py tests/test_v14_multijoint_stress_development_terminal.py tests/test_v14_multijoint_stress_qualification.py tests/test_v14_multijoint_stress_qualification_terminal.py
	$(PYTHON) scripts/freeze_v14_multijoint_stress_design_pilot.py --check
	$(PYTHON) scripts/freeze_v14_multijoint_stress_development.py --check
	@if [ -f results/proofalign_predictive_virtual_brake_v14_multijoint_stress_development_20260731_fresh1/pilot_evidence.json ]; then \
		$(PYTHON) scripts/run_v14_multijoint_stress_development.py --validate-results >/dev/null; \
		$(PYTHON) scripts/freeze_v14_multijoint_stress_development_terminal.py --check; \
	else \
		echo "Skipping v14 stress development result check: local-only evidence is absent"; \
	fi
	$(PYTHON) scripts/freeze_v14_multijoint_stress_qualification.py --check
	@if [ -f results/proofalign_predictive_virtual_brake_v14_multijoint_stress_qualification_20260731_fresh1/qualification_evidence.json ]; then \
		$(PYTHON) scripts/run_v14_multijoint_stress_qualification.py --validate-results >/dev/null; \
		$(PYTHON) scripts/freeze_v14_multijoint_stress_qualification_terminal.py --check; \
	else \
