...
项目已经从“最小双层 Boolean prototype”进入“攻击 workload 与防御 method 双轨推进，
CTDA reference runtime 已落地并开始统一 Python/Lean 时序语义”的阶段。
...
- Lean monitor 已改为在 accepted history 与当前 prefix 的拼接 trace 上求值，并拒绝跨 prefix
  timestamp rollback；Python monitor 也将 accepted events 与 episode nonce 纳入 state digest；
...
- 有界 accepted-trace accumulator，保证后续 prefix 扩展既有 history 而不是重置公式；
...
## 3. 并行 Track A：攻击 workload

攻击线在固定 GPU commit/worktree 上独立运行，不等待 method 分支：

1. 继续官方 SABER LoRA 的 task-failure、action-inflation、constraint-violation 批次；
2. 扩展 victim VLA、task/init 和 policy seed，并报告 transfer；
3. 保存不可变 instruction/proposal/trace artifact、配置、模型版本和 seed；
4. 将官方 `{per_episode: [...]}` 输出通过显式 converter 转成版本化 replay schema，不修改原始
   文件，也不在 loader 中偷偷折叠重复 episode；
5. 攻击结果只作为 threat workload，最终防御效果必须在相同 artifact 上配对评估。

攻击机运行期间不得同步 method 分支；纯攻击 runner 的 camera preprocessing、warmup、replan、
seed、early-stop、输出字段和文件命名冻结。

## 4. P0-A：统一时序语义与协议

在把 Lean 接入 online dispatch 前，先保证 Python reference 与 Lean 规范描述的是同一协议。

### 已完成

- Lean `MonitorState` 保存完整 accepted trace；monitor 在累计 trace 上检查 invariant、guarantee
  和 terminal event；
- Lean mission、authorization 和 monitor 显式携带并检查 episode nonce；
- 跨 prefix event timestamp 必须严格前进；monitor state 与当前 state digest 显式绑定；
- Python `ContractMonitorState` 将 episode nonce 和 accepted events 纳入 digest，拒绝 history
  rollback；
- runtime phase-event 推导可以使用 prior observed atoms，保证分散在不同 prefix 的 guarantees
  不会因局部重置而丢失；
- 新增 split-prefix completion、history digest 和 timestamp rollback 回归测试。

### 下一步

1. 定义内部 `ctda-wire-v1` canonical schema，不替换现有 CLI/episode JSON；
2. 统一 episode nonce、time unit、proposal index、typed temporal formula、truth value 和 monitor
   history；
3. 建立 Python/Lean golden differential corpus，覆盖 `safe_pending / complete / violated /
   inconsistent`；
4. Python reference 从当前 atom accumulator 扩展到与 Lean DSL 等价的 temporal evaluator；
5. 证明 trace concatenation、monitor-step preservation、complete soundness 和 phase-advance
   integrity。

## 5. P0-B：接入 Lean CTDA 在线 evaluator

在 P0-A 的 wire schema 和 differential parity 稳定后，让每次 online authorization 使用 Lean
中的 CTDA checker，而不是只运行 Python reference checker。
...
## 6. P1：从条件运动学界升级 physical certificate
...
- 纯攻击脚本不得导入 CTDA checker；共享 task/env/attack-record API 保持稳定；
- `--attack-record`、task/init/seed、camera、horizon、output 参数和现有 JSON key/enum 不删除；
- 新 evaluator 只通过 additive mode/schema 接入；`--ctda` 在迁移期保留为兼容入口；
- legacy `--skip-existing` 保持 resume 行为，CTDA 因需要 live revalidation 继续禁止；
- shared OpenPI policy 每个 episode 恢复初始 RNG，避免 task 遍历顺序改变 proposal；
- method merge gate 包含 full pytest、Lean build、三个 CLI `--help`、attack-record golden test 和
  wrapper/runner/batch smoke；GPU 合并前再跑一集真实 attack 与一集 legacy defense smoke。
