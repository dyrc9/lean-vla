...
2. **Python typed CTDA reference path。** 已有不可变 mission/contract/proposal/
   ...
   和 fail-closed verdict。monitor state 现将 episode nonce、accepted events 和最后事件时间
   纳入 digest，并拒绝跨 prefix timestamp rollback。
3. **Lean 三段 checker 与三值 monitor。** Lean 已实现并证明 semantic、prefix-pre、observed
   ...
   post evidence。`MonitorState` 保存 accepted symbolic history，后续判断在
   `acceptedTrace ++ currentPrefix` 上求值；mission/authorization/monitor 也显式绑定 episode
   nonce。
...
1. **Python runtime 尚未调用 Lean CTDA evaluator。** 当前 LIBERO 运行路径标记为
   ...
   evaluator，因而不能声称每次 LIBERO prefix 都由 Lean 判定。Python reference 当前仍使用
   string-atom accumulator，尚未与 Lean `TraceFormula` DSL 通过 canonical wire schema 和
   differential corpus 证明语义等价。
