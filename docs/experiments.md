...
用户随后授权一个新的、与旧 pair 分离的 R0b protocol。它在任何新 attack 前固定 clean-only task
顺序，排除 task 2，并对前三个 clean-success pair 完整执行三 family × 三 strength grid；同一 cell
至少在 2/3 pair 上产生 clean-success -> attacked-failure 才通过 primary signal gate。该授权不是
按旧 outcome 单独调到 strong：所有九个 cell 都必须按固定顺序运行和报告，不能提前停止。详细协议
与命令见 [`next_agent_prompt_20260715.md`](next_agent_prompt_20260715.md)。
...
一次预注册 Phantom R0b 与 SAFE/FIPER 上游复现；R0b 通过后才可预注册最小 R1 workload，不直接进入
60 episodes。
