...
| SAFE R0 | **官方 rollout 正在独立 worktree 运行** | source/checkpoint/tokenizer/三套 uv 环境已冻结；2026-07-15 17:02 快照为 175/500 env records | 等 500/500 后验证 env/policy count、schema 与 tree digest，再运行完整 detector matrix |
| FIPER R0 | **官方完整 pipeline 正在独立 worktree 运行** | 官方 archive/extracted tree/uv 环境已冻结；原始入口的 `np.bool` 失败已保留，最小兼容运行已越过失败点 | 等 terminal manifest，验证五 task × 五 seed × 全方法输出后再分类 |
...
hash 和 terminal checklist 见 [`safe_fiper_r0_runbook.md`](safe_fiper_r0_runbook.md)。表中的 175/500
只是带时间戳的 handoff 快照，不是最终结果。

当前执行顺序只有一条：让已启动的 SAFE/FIPER 官方 R0 原样到达 terminal 状态，保存全部失败尝试，
完成 target-specific validator，再决定 `reproduced_upstream` 或 `blocked_upstream`。在此之前不开发
pi0.5 baseline adapter、不恢复 Phantom/SABER，也不启动主表。
...
环境已在 `/data0/ldx` 冻结并复用，SAFE 官方 500-episode rollout 与 FIPER 官方完整 pipeline 正在运行；
两者仍无 terminal 结果，不能进入比较表。新的 held-out R1 与其后条件式 scoped main protocol
已在结果出现前冻结。
...
- SAFE/FIPER 独立复现已冻结 SAFE、SAFE-openpi、FIPER 三个 source commit，SAFE 12 GB checkpoint、
  ...
  输入均按绝对路径复用；当前只生成新的 rollout/training output。运行状态与恢复规则见
...
1. 历史 SABER 官方 LoRA/OpenPI R0 只在 constraint-violation 方向成立，仍不能冒充
   ...
   已完成但独立 safety signal 只有 1/4，未达到冻结的 2/4 gate。SAFE/FIPER 官方 pipeline 已恢复但
   仍在运行，尚无可用于比较的 terminal 结果；SABER exact-task R1 又在 immutable record-generation
...
1. 保留 Phantom 旧 task-2 R0 负结果和已完成的 R0b ledger；不重跑、不改攻击算法，
   不把 R0b 写成防御证据；
2. 保持独立 worktree 与现有进程不变，让 SAFE rollout 和 FIPER compatibility run 到达 terminal；
   不重新下载资产、不重建正在使用的 uv 环境、不覆盖首次 FIPER 失败；
3. SAFE 到 500/500 后先做 record/schema/count/tree-digest gate，再运行完整 detector matrix；FIPER 退出后
   先验证 manifest 与全 task/seed/method 输出。任一失败均按协议分类，不以 reduced smoke 替代；
4. Phantom R1 已按冻结协议完成并失败：1/4 safety transition < 2/4；按 failure next step 停止 Phantom，
   不调 `laser_blinding/strong`、不替换 pair、不追加 episode；
5. `proofalign_phantom_main_protocol.json` 的 prerequisite 未满足，而且唯一 unsafe event 位于 zero-based
   policy action 132，已超出其冻结的 100-action window；所以 scoped CTDA main 不运行；
6. SABER exact-task R1 已在第一个 pair 的 record-generation gate fail closed：无有效 record、无 victim
   outcome；不修 `CURRENT_CONFIG` 后重跑、不重生成/替换 record，条件式 scoped main 不运行；
7. SAFE/FIPER 未形成 terminal pass 前，不开发 pi0.5 adapter、不启动 60 episodes 或完整 Table B；
   若要开启新的攻击实验线，必须重新选择不同的、事前冻结的问题。
