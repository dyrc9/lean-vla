...
| ProofAlign 自身评测 | **当前主线，待冻结 E0 protocol** | 已有 correctness/parity/five-prefix 证据，但尚无跨任务 clean utility、独立 false-block label、双层消融或完整 task outcome | 先做支持范围审计，再做 VLA-only/Full CTDA clean paired pilot、fixed-trace 双层消融、closed-loop 与 cost；不等待 baseline |
...
| FIPER R0 | **fresh 后台复现异常退出，未复现** | 2026-07-16 fresh run 使用既有环境完成 seed 0 前三 task evaluation，并在 `pretzel/rnd_a` 训练结束后无 traceback、无 terminal manifest 退出；partial outputs 不计结果 | 先审计退出原因；若继续必须是新 fresh result directory，不 resume/拼接，且不能阻塞 ProofAlign E0--E4 |
...
截至 2026-07-16 13:23，FIPER fresh 进程已不在进程表，manifest 仍为 `started`，因此分类为异常
partial run 而不是完成。所有既有环境和下载资产继续复用，不新建环境；partial outputs、首次失败和
中断日志原样保留，不拼接结果。该后台异常不阻塞 ProofAlign E0--E4，但在新 fresh protocol 前不把
FIPER 接入最终比较。
...
1. **E0 support audit**：枚举 frozen compiler/runtime 对 LIBERO-Safety task/phase/init 的
   `supported / unsupported / ambiguous`，在新 outcome 前冻结 self-evaluation protocol；
2. **E1 clean paired pilot**：同一 task/init/env seed/policy seed 配对 VLA-only 与 Full CTDA，先测
   task/safe success、coverage、retention、false block、unknown/deadlock 与 phase completion；
3. **E2 duality**：对同一保存 trace 运行 mission-only、trace-only、Full CTDA，报告各层 unique catch、
   交集、parity 与 clean loss；
4. **E3/E4**：再测 closed-loop dispatch/phase intervention、fallback postcondition、最终 task outcome、
   latency、deadline miss、artifact 和 CPU/GPU/内存开销；保持 slow-interlock claim；
5. FIPER 只属于后台 external lane。2026-07-16 fresh attempt 无 terminal pass；继续前先审计异常，并且
   只能新建 result directory、复用现有环境，不 resume 或拼接。SAFE 继续 deferred；
6. Phantom R1 的 1/4 负结果和 SABER record-generation fail-closed 永久保留，不调 gate、不换 pair、
   不重开两个条件式 scoped main；
7. 只有 E0--E4 已形成自身结果且 external readiness 通过后，才做 SAFE/FIPER/privileged checker 的
