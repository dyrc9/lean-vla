...
| ProofAlign 自身评测 | **E1-v3 terminal-invalid；E3 clean 12/12 preserved；post-dispatch primary 12/12 unknown** | post-dispatch 12/12 valid，均实际 monitor-unknown、零 phase advance、replan、zero-hold 和 complete-safe postcondition；但 frozen labeler 要求 receipt 不存在的顶层 integrity bool，正式为 0 contained / 0 failed / 12 unknown | 不改写 E1/E3、不修 labeler 后重跑；post-hoc typed receipt 12/12 integrity pass 仅作诊断，不能升级主分类；下一步转 E4 fail-closed robustness，仍不声称 recovery/attack defense |
...
3. **post-dispatch challenge 已终止**：12 条有效 artifact 全部保留，正式 primary 为 12 unknown；不修
   labeler 后重跑，不以 post-hoc typed receipt audit 升级。安全主线下一步转 E4 的 Lean unavailable、
   timeout、artifact/receipt tamper fail-closed robustness；timing 只留原始诊断；
