...
| ProofAlign 自身评测 | **E0 v2 已冻结 12 个 unit；E1-v3 terminal-invalid，0 valid pair** | v3 24/24 artifact 落盘；12/12 VLA-only valid，12/12 Full CTDA 在 post-dispatch paired validator 因观察 schema 不同导致 init digest mismatch 而 invalid；inference 正确为 `not_evaluated` | 不 resume/覆盖 v1/v2/v3，不替换已 dispatch pair，不进入 E2；可另行设计未来共享 observer 协议或进入独立 E4 cost audit |
...
3. **独立 E4**：在不恢复 E1 claim 的前提下审计既有 verifier/system timing、artifact 和资源开销，再测
   closed-loop dispatch/phase intervention、fallback postcondition、最终 task outcome、
