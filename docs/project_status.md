...
| FIPER R0 | **收尾时中断，未复现** | 原始 `np.bool` 失败已保留；compat run 停在 seed 0 / `push_t` / `rnd_oe`，无 terminal manifest | 保留日志与 partial outputs；未来如重启必须用 fresh result directory 跑完整矩阵 |
...
环境已在 `/data0/ldx` 冻结并复用。两条长任务随后在 2026-07-15 收尾时中断：SAFE 保留
335/500 records，FIPER compat run 停在 seed 0 / `push_t` / `rnd_oe`；两者都无 terminal manifest，
不能进入比较表。新的 held-out R1 与其后条件式 scoped main protocol
已在结果出现前冻结。
...
1. 历史 SABER 官方 LoRA/OpenPI R0 只在 constraint-violation 方向成立，仍不能冒充
   ...
   已完成但独立 safety signal 只有 1/4，未达到冻结的 2/4 gate。SAFE/FIPER 官方 pipeline 均在
   terminal 前中断，尚无可用于比较的结果；SABER exact-task R1 又在 immutable record-generation
...
1. 保留 Phantom 旧 task-2 R0 负结果和已完成的 R0b ledger；不重跑、不改攻击算法，
   不把 R0b 写成防御证据；
2. SAFE/FIPER 已停止且无残留进程；保留 335-record SAFE partial corpus、FIPER partial outputs、
   首次失败和中断日志，不重新下载资产、不拼接为完整 run；
3. 当前不授权续跑。未来如重启，必须先决定 SAFE fresh/resume protocol，并让 FIPER 在 fresh result
   directory 完整重跑；仍须通过 manifest、schema、digest 与全 task/seed/method gate；
