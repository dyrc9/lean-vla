...
- 当前全量为 280 passed / 1 skipped，Lean `lake build ProofAlign` 为 12 jobs 成功；本轮新增
...
1. 历史 SABER 官方 LoRA/OpenPI R0 只在 constraint-violation 方向成立，仍不能冒充
   ...
   SABER exact-task R1 又在 immutable record-generation artifact gate 失败且没有 victim outcome。
...
5. SABER exact-task R1 已在第一个 pair 的 record-generation gate fail closed：无有效 record、无 victim
   outcome；不修 `CURRENT_CONFIG` 后重跑、不重生成/替换 record，条件式 scoped main 不运行；
6. 不启动 60 episodes 或完整 Table B。若要开启新的实验线，必须重新选择不同的、事前冻结的问题；
   本次 SABER R1 不得被复活或解释为攻击/防御结果。
