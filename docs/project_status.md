...
- 当前全量为 269 passed / 1 skipped，Lean `lake build ProofAlign` 为 12 jobs 成功；本轮新增
...
5. 不启动 60 episodes 或完整 Table B。下一条实验线需要用户重新选择：恢复 SAFE/FIPER，或为已部分
   复现的 SABER 单独设计 outcome-blind LIBERO-Safety R1；在新选择前只做 R1 artifact/status 收尾。
