...
官方上游复现 gate，不是 60-episode 或主实验。
...
1. 历史 SABER 官方 LoRA/OpenPI R0 原始记录已经重新核验：constraint-violation 方向成立，
   ...
   LIBERO-Safety exact-task R1。Phantom Menace、SAFE 和 FIPER 尚未完成官方 pipeline。
...
1. 将已核验的 SABER R0 原始 records、upstream commit、local patch 和偏差写入机器清单；
2. 获取并运行 Phantom Menace 官方 clean + deterministic camera-transform smoke，先完成 R0；
3. R0 通过后才为 exact LIBERO-Safety task 生成 SABER instruction 与 Phantom camera R1 workload；
