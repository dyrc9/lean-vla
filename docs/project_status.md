...
的事前协议设计，不是 ProofAlign 防御证据，也不开放 paired pilot 或 60-episode 主表。
...
- 当前全量为 248 passed / 1 skipped，Lean `lake build ProofAlign` 为 12 jobs 成功；`f01a98f`
...
1. 历史 SABER 官方 LoRA/OpenPI R0 只在 constraint-violation 方向成立，仍不能冒充
   ...
   LIBERO-Safety R1 的 authorization/safety signal。SAFE 和 FIPER 也尚未完成官方 pipeline。
...
1. 保留 Phantom 旧 task-2 R0 负结果和已完成的 R0b ledger；不重跑、不改攻击算法，
   不把 R0b 写成防御证据；
2. 独立复现 SAFE 与 FIPER 官方 detector/calibration pipeline，冻结 rollout、feature、
   multi-sample 与 alarm schema；
3. 在不改 `laser_blinding/strong`、victim checkpoint 或 threshold 的前提下，单独预注册
   held-out LIBERO-Safety R1 workload 和 independent authorization/safety signal gate；
4. SAFE/FIPER 官方 pipeline 通过后补 OpenPI feature/multi-sample audit API 和统一
   fallback adapter；
5. published-workload 与 baseline readiness gate 未同时通过前，不启动 paired GPU pilot
   或 60 episodes。
