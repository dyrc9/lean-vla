...
| ProofAlign 自身评测 | **E0 v2 已冻结 12 个 unit；E1-v3 preflight 已通过，尚无有效 paired result** | v1/v2 均为 0-valid invalid evidence；v3 已隔离 metadata deep-copy、修复 valid-pair inference，并在 GPU 3 通过真实 OpenPI output audit，`env.step_called=false` | 只可用 v3 新目录 fresh 执行相同 12 units；不 resume/覆盖 v1/v2，获得有效 paired artifact 前不进入 E2 |
...
冻结 hash 不变；同时仅允许两侧都 valid 的 pair 进入 inference。GPU 3 preflight 已通过 exact
CUDA/EGL import、15-task registry、checkpoint 和真实 OpenPI policy-output audit；探针审计 10 个完整
action、metadata SHA-256 为 `f339032e9190e40443a8273ecdcfc8c029c46954f5365bf1bef5d334108bd0bb`，
且未调用 `env.step()`。正式 v3 fresh root 尚未创建，因此当前仍是 0 valid E1 result。
...
1. **E1-v3 repair 与 fresh pilot**：保留 v1/v2 invalid artifacts；为真实 OpenPI nested metadata 增加
   deep-copy/serialization fixture，禁止 invalid pair 进入 bootstrap/McNemar；focused/full/Lean gate
   通过后冻结 amendment，再对相同 12 units 使用全新目录运行 VLA-only / Full CTDA；
2. **E2 duality**：对同一保存 trace 运行 mission-only、trace-only、Full CTDA，报告各层 unique catch、
   交集、parity 与 clean loss；
3. **E3/E4**：再测 closed-loop dispatch/phase intervention、fallback postcondition、最终 task outcome、
