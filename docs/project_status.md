...
绑定；完整验证、clean preflight 与唯一一次固定 calibration 尚待完成，后续大实验仍关闭。
...
- 当前全量为 227 passed / 1 skipped，Lean `lake build ProofAlign` 为 12 jobs 成功。
...
3. `7587c47` method-validity gate 已越过历史 timing blocker，但暴露 dynamics config mismatch：
   ...
   `model_error_m=0.0001 m` 继续只作为 tube 的独立误差 allowance。
...
5. 旧 notes 中的 60-episode baseline、12-episode Dual Lean、SABER 和 EDPA 结果没有完整 raw
   artifact 保存在当前 checkout，不能仅凭叙述重建主表。
6. 当前本地旧 heuristic artifact 出现极高 false rejection，且 synthetic golden corpus 没有独立
...
1. 完成 live-controller binding 与 command-path-derived `0.004 m` budget 的全量 CPU/Lean 验证、
   clean commit 和 strict preflight；
2. 只运行一次相同 task/init/seed/witness 的 3--5 prefix calibration，并逐 prefix 核对 controller
   digest、累计双预算、tube、proof/parity、phase 与完整 action-chunk audit；
3. 仅在该 gate 真正通过后，按
...
- repeated cumulative-stutter 的 reset/replan/多 prefix 边界目前只有 CPU/fake-env 证据；真实 run 只
