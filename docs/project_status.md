...
- 当前全量为 220 passed / 1 skipped，Lean `lake build ProofAlign` 为 12 jobs 成功。
...
3. `7587c47` method-validity gate 已越过历史 timing blocker，但暴露 dynamics config mismatch：
   ...
   translation budget 0.1 mm。修正 live-controller binding 属于 correctness，扩大 stutter contract
   则是新的方法选择，不能借本次样本静默调大。
...
1. 将 translation scale 从硬编码改为绑定 live `OSC_POSE` controller config，并对缺失、非各向同性
   或不一致 config fail closed；
2. 在不使用本次 verdict 拟合阈值的前提下，单独决定是否保持 0.1 mm stutter translation budget
   （会 pre-dispatch refute 首 prefix）或以预先声明的物理/安全依据修订合同；
