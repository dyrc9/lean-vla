...
Figure~\ref{fig:overview} summarizes the runtime path.  The policy and monitor never share authority: policy-facing bytes are evidence about what the VLA consumed, whereas $T$, $O_t^T$, and $Z_t$ determine the monitor context.  The output of one policy call is a single source \actionblock.  \system does not resample the policy, ask the policy to explain itself, or relabel $Z_t$ after seeing the action.

The design follows four ordered invariants.  First, an action receives no
covered-risk authorization without an assessment bound to a trusted context.
Second, a dispatcher accepts no command without a fresh authorization bound to
that exact action and state epoch.  Third, execution cannot advance the task
phase until matching receipts and effects close the transaction.  Fourth, a
near-boundary action is dispatched only under a guard configuration that
passes the registered margin, restore, identity, and force gates.  Later
layers preserve earlier identities rather than replacing their verdicts.
...
\subsection{C1: Trusted Semantic Context}
...
\subsection{C2: L1 Checker-Relative Assessment}
...
\subsection{C3: L2a One-Use Execution Transaction}
...
\begin{figure}[t]
\centering
\resizebox{\columnwidth}{!}{\input{figures/execution_transaction}}
...
\subsection{C4: L2b Bounded Guard Screening}
