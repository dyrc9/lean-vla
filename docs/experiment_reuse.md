...
- 旧实验没有 prospectively generated `ActionBlockAssessment` 和
  `BlockExecutionContract`，因此不能改名为 Semantic-only、Dual 或 Lean-verified v3 结果。
...
- 与新 Semantic-only/Dual episode 拼成一个四臂 factorial；
- 将 R9 作为新四臂的 confirmatory Execution-only cell；
- 声称一般防御或完整物理安全；
- 因为 envelope bound 成立就声称 task intent alignment。

新的 `K=1` primary 四臂必须在同一个 shared runner、相同 ActionBlock bytes 和相同
assessor/contract 下运行；R9 没有这些并行 treatment identities。任何 `K>1` 扩展则必须共享完整
candidate set，并把 L1 selection 明确作为 treatment mechanism。
