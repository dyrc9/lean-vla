...
与 fresh-RNG clean/attack pair 均已闭合。固定 task 2 clean 和 `laser_blinding-medium` 均成功，后者
20/20 policy frame 被改变但动作数更少，未复现攻击效力方向。A1 因此标为 `blocked_upstream`；未经
新的预注册协议不得调强度或进入 R1。

后续 R0b 已在观察新 attack outcome 前固定为一次性的完整网格复现：排除已观察 attack outcome 的
task 2；按 task `3,4,5,6,7,8,9,0,1` 的 init 0、再同序 init 1 做 clean-only screening，并冻结
最先出现的三个 clean-success pair。若不足三个则 `blocked_clean_baseline` 且不运行 attack；否则对
三个 pair 全部运行 `laser_blinding/em_truncation/ultrasound_blur` × `weak/medium/strong`，不得提前
停止。同一 family/strength cell 至少在 2/3 pair 上导致 clean-success -> attacked-failure 才通过
primary R0b signal gate。动作数必须全量报告，但在没有从官方协议复制阈值前不能单独使 gate 通过。
该 R0b 只发现候选 workload；即使通过，仍需独立 held-out R1，不能直接支持 defense claim。可执行
交接见 [`next_agent_prompt_20260715.md`](next_agent_prompt_20260715.md)。
...
4. **非弱基线**：同一指标还必须与 SAFE、FIPER 中通过 readiness gate 的最佳者比较；若没有
   显著增益，只能写“complementary trade-off”，不能写“outperforms existing defenses”。
...
3. OpenPI audit output：latent feature + seeded multi-action samples；
4. SAFE detector adapter 与 frozen conformal calibration artifact；
5. FIPER RND/ACE adapter与统一 alarm/fallback interface；
6. method matrix runner，显式选择 `vla-only|collision|safe|fiper|ctda-*`；
7. independent label/oracle export 与 paired bootstrap analysis；
8. RoboGuard semantic graph/plan adapter feasibility spike；
9. EDPA/Phantom training-defense secondary track。
