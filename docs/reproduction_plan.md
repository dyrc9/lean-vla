...
本文定义最终外部 workload/baseline comparison 的复现线。它与 ProofAlign 自身评测并行，
不再阻塞 clean utility、duality、closed-loop semantics 或 runtime cost。它定义复现对象、接入边界和
进入比较表的 gate；具体指标与统计协议仍以
[`experiments.md`](experiments.md) 为准，远程命令与路径以
[`remote_execution.md`](remote_execution.md) 为准。

## 1. 两条并行证据链

论文不能从 synthetic fixture 直接跳到“方法有效”，也不能让外部 baseline readiness 卡住我们自己
的测量。当前依赖改为：

```text
ProofAlign lane: E0 scope -> E1 clean utility -> E2 duality -> E3 closed loop -> E4 cost
                                                                            \
External lane:   R0 upstream -> R1 qualified workload -> R2 adapter/readiness -> E5 comparison
```

E0--E4 不依赖 R0--R2；它们先回答 supported slice 内的 correctness、utility、duality、intervention
和 cost。R0 失败时不能继续把外部工作列为“已复现”；R1 只造成 task failure、没有
...
### Phase R2：adapter/readiness 与最终 fixed-trace comparison 准备
...
2026-07-16 priority update：完整 R2/E5 external comparison 不在当前主线；ProofAlign E0--E4 可独立
执行。只有 R1 与 baseline readiness 通过后才运行 external comparison；不得把自身评测写成
related-work comparison 或“优于现有防御”。
...
执行结果：四次 pre-generation runtime failure 均在零 pair attempt 下审计恢复；第五次通过 server
health gate 后，第一个 pair 的 `init_chat_model()` 因 ART `CURRENT_CONFIG` context 缺失失败。invalid
transcript/producer-ledger 已持久化，perturbed instruction 未改变、edit distance 0、tools 为空，且无
victim load/rollout。按照事前 `invalid_record_policy`，不能修复 producer 后重跑或替换该 record。
因此 R1 分类为 `r1_saber_attack_record_generation_failed_closed`；2/4 signal 与 ProofAlign method
validity 均未评估，条件式 scoped main 不执行。

### Phase E5：最终 paired closed loop comparison
...
4. **非弱基线**：该条只约束最终 external comparison，不约束 ProofAlign E0--E4。SAFE 尚无 terminal
   pass；FIPER fresh R0 仍须通过 terminal gate；
...
当前 external lane 顺序改为：

1. FIPER 使用已有环境按 fresh R0 protocol 后台复现；异常/partial run 不拼接为 pass；
2. SAFE 保持 deferred，不阻塞 ProofAlign self-evaluation；
3. Phantom R1 已以 1/4 < 2/4 关闭，SABER R1 已 fail closed；两条 scoped main 均不重开；
4. ProofAlign E0--E4 完成且 external readiness 通过后，才做 adapter、frozen calibration 与 E5 comparison；
5. RoboGuard、EDPA/Phantom training-defense 继续作为 secondary track。

在完整 baseline 与统计 gate 完成前，不启动大规模 comparison table；这不阻止小规模、事前冻结的
ProofAlign E0--E4 自身评测。
