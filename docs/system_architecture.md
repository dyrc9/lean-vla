...
上述五组件已分别由 `FrozenMissionAdapter`、`PersistentContractMonitor`、`ExactPrefixAuthorizer`、
`SingleDispatchBoundary` 和 `EffectObserverUpdater` 实现。当前 boundary 只接
`InMemoryCommandSink`；它记录 typed receipt，但没有 simulator、socket、GPU 或 hardware action capability。
...
| 当前代码 | 当前事实 | 后续定位 |
|---|---|---|
| `src/proofalign/integrity_models.py` | 独立 method/schema、五类核心对象、四臂/verdict/transaction types | minimal core domain model |
| `src/proofalign/integrity_checker.py` | typed intent equality、fresh transaction binding、exact final-command authorization | Python deterministic fast checker；尚无 Lean refinement artifact |
| `src/proofalign/integrity_runtime.py` | 五组件、三 transaction、one-use boundary、receipt/effect 与 atomic monitor update | 当前仅 in-memory no-action prototype |
| `src/proofalign/integrity_intervention.py` | pass/project-or-brake/replan/hard-block 可替换 policy | optional intervention；adjusted command 回到 authorizer |
...
| `lean/ProofAlign/IntegrityCore.lean` | 四臂 semantics、dual dispatch 前提、exact execution binding、checked completion theorem | minimal Lean core；尚未连接 Python fast checker/wire |
| `lean/ProofAlign/CTDA*.lean` | v1/v2 typed checks、theorem 与 replay | 历史形式语义资产 |

新原型不导入 v2 core/wire，也不修改 v1/v2 artifact。旧 class/file 继续作为历史兼容路径，不能反过来
定义新方法。
...
- `proofalign-integrity-v1`：Python deterministic checker + in-memory no-action sink；实现三 transaction 与
  四臂 switch，但没有 simulator adapter、online wire 或 Lean/Python refinement claim。
...
1. **Architecture refreeze**：五组件、三个 transaction、四个 arm 与独立 method/schema id；
2. **Core implementation**：deterministic Python checker、one-use boundary、checked completion/atomic monitor；
3. **Core formalization**：Lean 中两个不变量、四臂 layer semantics 与 exact execution binding；
4. **No-action transaction tests**：wrong target、stale state、command substitution、false completion、
   post-filter command、unknown evidence、replay 和 no-phase-advance。

仍未完成且当前不得作为实验执行：

1. Python fast checker 到 Lean core 的 machine-checked refinement/equivalence；
2. strict three-transaction online wire 与 shadow evaluator；
3. benchmark Mission Adapter、trusted observer 和 simulator Single Dispatch Boundary integration；
4. fixed-trace four-arm outcome、clean closed-loop gate 和 qualified attack comparison；
5. AEGIS/CBF、brake/replan/recovery 和外部 detector/semantic baseline。
