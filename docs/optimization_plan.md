...
当前所有远端实验与 rollout 暂停。本文仍定义恢复实验后的第一顺序：先完成 unguarded VLA-only 发布
攻击复现与 threat qualification。用户另行授权了本地 `proofalign-integrity-v1` 最小原型、unit tests 和
Lean build；该授权不包含 fixed-trace outcome、simulator/GPU、clean method pilot、AEGIS/SAFE/FIPER 或
attacked+defended comparison。
...
安全实验基础和 CTDA v2 no-action 结果全部冻结。下面是恢复实验后的单线顺序，当前不执行：
...
不创建任何实验 output。恢复实验时，只有阻断 official attack producer、VLA-only victim 或独立 safety
oracle 的代码/环境问题允许修复；VLA-only terminal 结果形成后仍必须停止并请求下一步授权。

## 2. 阶段目标与非目标

当前代码目标是完成并审计第 3 节的 no-action minimal prototype；所有实验暂停。第 4 节仍是恢复后的
第一实验，第 5 节实验矩阵和 D2--D4 继续 deferred；新原型不继承现有 CTDA v2 六阶段架构。
