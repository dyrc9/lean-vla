# Action-envelope attacked+defended 优先规划

更新日期：2026-07-23

## 0. 文档地位

P0b fresh2 已完成 48-record producer 和 96/96 valid clean/attacked victim episode，但 23 个
clean-eligible pair 低于冻结的 26-pair gate，正式分类保持
`p0b_blocked_insufficient_clean_baseline`。用户明确指定当前唯一实验优先级为 exploratory Execution-only
action-envelope attacked+defended successor。

clean R1 retention 为 `22/23 = 0.9565`；attacked R2 在任何新 `env.step` 前因 non-finite command terminal。
R3 zero-brake attempt 通过 static preflight，但 runtime GPU contention 与实际 JAX/EGL mapping 异常使
all-or-nothing binding probe 超过三小时仍未完成，随后按用户要求停止；0 episode/outcome。R3 root 不得续跑。
下一执行必须使用 resource-isolated successor protocol 和 fresh root，完整 checkpoint 见
...
安全实验基础和 CTDA v2 no-action 结果保持冻结。当前授权的单线顺序是：

```text
preserve stopped R3 root
  -> repair and test actual JAX/EGL device isolation
  -> freeze successor protocol/source hashes/fresh root
  -> wait for two stable, distinct, idle GPUs
  -> complete all-or-nothing binding probe
  -> run 48 attacked+defended episodes
  -> terminal manifest/ledger/summary/checksums/validator
  -> stop and report
```
...
- 只执行当前 action-envelope successor attacked+defended comparison；
...
resource-isolated action-envelope successor 是唯一允许创建新实验 output 的路径。EDPA、Full CTDA、
AEGIS、SAFE、FIPER 和其他 method arm 不并行启动。

## 2. 阶段目标与非目标

当前代码目标除第 3 节 no-action minimal prototype 外，只增加 current action-envelope successor 所需的
device isolation、runtime gate 和 terminal audit。第 5 节完整四臂矩阵和 D2--D4 继续 deferred。
...
### M5：P0b substrate 与 action-envelope successor

**状态：P0b fresh2 complete/nonqualified；R1 clean pass；R2 invalid；R3 resource-stopped；successor current
priority。** P0b 完成 48 record 与 96 episode，但 clean-eligible `23 < 26`。R1 clean retention 为
`22/23`。R2/R3 均未产生 attacked+defended outcome。EDPA P1a 已在 policy/simulator/episode 前 terminal。
下一步只准备并执行 resource-isolated action-envelope successor。
