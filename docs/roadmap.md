...
4. SABER P0 已从全新的 official producer gate 完成至 R7 terminal；R4--R7 artifact 不续接；
...
7. 后续 P1 使用原始 EDPA patch 叠加 SafeLIBERO；不按 outcome 调 patch、task、seed 或强度。它必须有
   自己的 frozen protocol/runner/asset gate，不能执行历史 EDPA R0 草案；
...
9. 每条 workload 都保存 terminal manifest、append-only ledger、episode artifact 和 SHA-256；若均无攻击
